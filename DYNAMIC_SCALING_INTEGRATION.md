...
## Trainer & CLI Integration (Latest)

### Files Modified

1. **`src/models/trainer.py`** - Core training with dynamic scaling
   - Added `scaling_config` parameter to `EmbeddingTrainer.__init__`
   - Implemented `train_with_dynamic_scaling()` method for continual learning
   - Tracks metrics: buffer size, memory usage, exemplars per cluster
   - Auto-calculates buffer size from comprehensive stats

2. **`scripts/train_model.py`** - CLI interface
   - New arguments: `--enable-scaling`, `--memory-budget`, `--target-accuracy`
   - Additional: `--epochs-per-task`, `--replay-ratio`
   - Conditional training: uses scaling or standard mode
   - Saves scaling metadata in output

3. **`src/ui/pages/training.py`** - Streamlit UI
   - Real-time buffer metrics from `scaling_history`
   - Calls `train_with_dynamic_scaling()` when enabled
   - Displays actual memory usage and buffer stats

4. **`test_dynamic_scaling_training.py`** - Integration test (NEW)
   - End-to-end validation of trainer integration
   - Tests 5 clusters, 250 sequences
   - Verifies buffer statistics, metrics tracking, embedding extraction
...
python scripts/train_model.py \
    ...
    --epochs-per-task 15 \
    --replay-ratio 0.3 \
    --input-file data/sequences.fasta \
    --output-dir models/trained
```

### Buffer Size Calculation

Handles different stats structures from `HybridMemoryBuffer`:
```python
stats = buffer.get_comprehensive_stats()
total = (
    stats['exemplar'].get('total_exemplars', 0) +  # ExemplarBuffer
    stats['uncertainty'].get('size', 0) +           # UncertaintyBuffer
    stats['recent'].get('size', 0)                  # ReservoirBuffer
)
```
...
✅ ALL TESTS PASSED!
Summary:
  • Model trained on 250 sequences across 5 clusters
  • Final buffer size: 325 samples
...
**Run test**: `python test_dynamic_scaling_training.py`
...
- `test_dynamic_pipeline_integration.py` - Integration test (pipeline)
- `test_dynamic_scaling_training.py` - Integration test (trainer/CLI)
