...
def test_model_memory(model, embeddings, cluster_labels):
    ...
    for cluster_id in range(n_clusters):
        ...
        with torch.no_grad():
            ...
            print(
                f"  Accuracy: {accuracy:>6.1f}% ({correct:>4}/{len(cluster_indices):>4})"
            )
    ...
    clusters_partial = sum(
        1 for r in cluster_results.values() if 50 < r["accuracy"] <= 75
    )
    ...
    print(
        f"   ⚠️  Partially Retained (50-75%): {clusters_partial}/{n_clusters} clusters"
    )
    ...
    for cluster_id in range(n_clusters):
        ...
        sample_indices = np.random.choice(
            cluster_indices, min(3, len(cluster_indices)), replace=False
        )
        ...
        with torch.no_grad():
            for idx in sample_indices:
                ...
                print(
                    f"  Sample {idx}: {status} Predicted C{predicted_cluster} "
...
def test_incremental_forgetting(model, embeddings, cluster_labels):
    ...
    for position, cluster_id in enumerate(training_order):
        ...
        with torch.no_grad():
            ...
            print(
                f"  Position {position} (C{cluster_id}): {accuracy:>6.1f}% {time_str}"
            )
    ...
    if late_avg > early_avg + 5:
        print(
            f"   ⚠️ Recent bias detected: Later clusters have {late_avg - early_avg:.1f}pp higher accuracy"
        )
        print(f"   This suggests some forgetting of earlier knowledge.")
    elif early_avg > late_avg + 5:
        print(
            f"   ℹ️ Early advantage: Earlier clusters have {early_avg - late_avg:.1f}pp higher accuracy"
        )
        print(f"   This could indicate stronger consolidation of early knowledge.")
    else:
        print(
            f"   ✅ Balanced retention: Difference only {abs(late_avg - early_avg):.1f}pp"
        )
