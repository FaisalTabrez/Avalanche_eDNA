...
for epoch in range(3):
    ...
    print(f"      Epoch {epoch}: loss={loss:.3f}, acc={accuracy:.3f} → Saved to {Path(checkpoint_path).name}")
...
if best:
    print(f"   ✓ Best checkpoint: epoch {best['epoch']}, loss={best['metrics']['loss']:.3f}")
print()
...
for model_info in models:
    print(f"      - {model_info['version']}: {model_info.get('description', 'No description')}")
...
learner_replay = ContinualLearner(
...
print(f"      ✓ Computed Fisher information for {len(learner_ewc.fisher_dict)} parameters")
...
print(f"   - Best checkpoint: epoch {checkpoint_manager.get_best_checkpoint_info('loss', minimize=True)['epoch']}")
print()
...
latest_lineage_versions = [v['version'] if isinstance(v, dict) else v for v in latest_lineage]
print(f"   - Training lineage: {' → '.join(latest_lineage_versions)}")
print()

print("✅ Continual Learning:")
print(f"   - Replay buffer capacity: {len(integration_learner.replay_buffer)}/{integration_learner.buffer_size}")
