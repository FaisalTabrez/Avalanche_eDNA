...
class EDNAAnalysisPipeline:
        ...
        if self.finetuner is None:
            ...
            for i in range(0, len(self.sequences), batch_size):
                ...
                if (i + batch_size) % 100 == 0:
                    print(f"  Processed {i + batch_size}/{len(self.sequences)} sequences")
    ...
    def cluster_sequences(self, n_clusters: int = 5) -> Dict:
        ...
        results = {
            ...
            'cluster_avg_lengths': {k: v['avg_length'] for k, v in cluster_info.items()},
    ...
    def visualize_clusters(self, cluster_labels: np.ndarray):
        ...
        plt.tight_layout()
        plt.savefig(self.output_dir / "visualizations" / "length_distribution.png", dpi=150)
        plt.close()
    ...
    def run_full_pipeline(self):
        ...
        print(f"  • Clustering quality (silhouette): {clustering_results['silhouette_score']:.3f}")
