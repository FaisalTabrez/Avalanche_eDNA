...
class SRAProcessor:
    ...
    def process_sra_fastq(
        self, fastq_path: Path, output_dir: Optional[Path] = None
    ) -> List[SeqRecord]:
        """
        ...
        logger.info(
            f"Processed {len(processed_records)} eDNA sequences, saved to {output_file}"
        )
    ...
    def _filter_edna_sequences(self, sequences: List[SeqRecord]) -> List[SeqRecord]:
        ...
        logger.info(
            f"Filtered {len(sequences)} sequences to {len(filtered)} eDNA-relevant sequences"
        )
        return filtered

    def extract_edna_markers(
        self, sequences: List[SeqRecord], marker_genes: Optional[List[str]] = None
    ...
    def create_sra_analysis_report(
        self, sra_accession: str, processed_sequences: List[SeqRecord], output_dir: Path
    ) -> None:
        """
    ...
    def integrate_with_pipeline(
        self, sra_fastq_files: List[Path], output_dir: Optional[Path] = None
    ) -> Dict[str, List[SeqRecord]]:
        """
        ...
        logger.info(
            f"Integrating {len(sra_fastq_files)} SRA files with analysis pipeline"
        )
        ...
        for fastq_file in sra_fastq_files:
            ...
            # Process SRA FASTQ
            processed_seqs = self.process_sra_fastq(
                fastq_file, output_dir / sra_accession
            )
            ...
            if processed_seqs:
                ...
                self.create_sra_analysis_report(
                    sra_accession, processed_seqs, output_dir
                )

                sra_metadata[sra_accession] = {
                    "processed_sequences": len(processed_seqs),
                    "marker_genes": {
                        gene: len(seqs) for gene, seqs in marker_sequences.items()
                    },
        ...
        logger.info(
            f"SRA integration complete. Processed {len(all_processed_sequences)} sequences from {len(sra_fastq_files)} files"
        )
...
def main():
    ...
    parser.add_argument(
        "--batch-process", nargs="+", help="Process multiple FASTQ files"
    )
    ...
    try:
        if args.batch_process:
            ...
            results = processor.integrate_with_pipeline(
                fastq_files, Path(args.output_dir) if args.output_dir else None
            )
            print(f"Processed {results['results']['total_sra_files']} SRA files")
            print(
                f"Generated {results['results']['total_processed_sequences']} processed sequences"
            )
        else:
            fastq_path = Path(args.fastq_file)
            sequences = processor.process_sra_fastq(
                fastq_path, Path(args.output_dir) if args.output_dir else None
            )
