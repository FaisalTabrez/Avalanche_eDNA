...
class SequenceQualityFilter:
    ...
    def filter_fastq(self, input_file: Path, output_file: Path) -> Dict[str, int]:
        ...
        logger.info(
            f"Filtering complete. {stats['final_passed']}/{stats['total_sequences']} sequences passed"
        )
        return stats
    ...
    def trim_adapters(self, input_file: Path, output_file: Path) -> bool:
        ...
        try:
            ...
            cmd = [
                ...
                "20",
                "--times",
                "2",  # Remove adapters up to 2 times
                "--output",
                str(output_file),
            ]
...
class PreprocessingPipeline:
    ...
    def __init__(self):
        ...
        self.quality_filter = SequenceQualityFilter(
            ...
            max_n_bases=preprocessing_config.get("quality_filter", {}).get(
                "max_n_bases", 5
            ),
        )
        ...
        self.processed_dir = Path(
            self.config.get("data.processed_dir", "data/processed")
        )
    ...
    def process_file(self, input_file: Path, output_prefix: str) -> Dict[str, Any]:
        ...
        if self.chimera_detector.detect_chimeras(fasta_file, final_file):
            ...
            logger.warning(
                f"Chimera detection failed for {input_file}, using filtered sequences"
            )
    ...
    def process_directory(
        ...
        for fastq_file in fastq_files:
            ...
            try:
                ...
                results.append(
                    {"input_file": str(fastq_file), "error": str(e), "completed": False}
                )
                ...
                row.update(
                    {
                        ...
                        "pass_rate": stats.get("final_passed", 0)
                        / max(stats.get("total_sequences", 1), 1),
                    }
                )
...
def main():
    ...
    parser.add_argument(
        "--input", type=str, help="Input directory containing FASTQ files"
    )
    parser.add_argument(
