...
class AdvancedTaxonomicAnalyzer:
    ...
    def analyze_taxonomic_composition(
        self, sequences: List, confidence_threshold: float = 0.8
    ) -> Dict[str, Any]:
        """
        ...
        results["taxonomic_distribution"] = self._calculate_rank_distributions(
            assignments
        )
        ...
        results["novel_candidates"] = self._identify_novel_candidates(
            assignments, confidence_threshold
        )
        ...
        results["phylogenetic_insights"] = self._analyze_phylogenetic_patterns(
            assignments
        )
            ...
            if assignment_prob > 0.15:  # 85% get some assignment
                ...
                for rank in self.taxonomic_ranks:
                    if rank in template:
                        ...
                        confidence > 0.6
                    ):  # Add more specific ranks for high confidence
    ...
    def _calculate_rank_distributions(self, assignments: List[Dict]) -> Dict[str, Dict]:
        ...
        for rank in self.taxonomic_ranks:
            ...
            rank_distributions[rank] = {
                ...
                "top_taxon": (
                    rank_counts.most_common(1)[0] if rank_counts else ("Unknown", 0)
                ),
                "assignment_rate": (
                    total_assigned / len(assignments) if assignments else 0
                ),
            }
    ...
    def _identify_novel_candidates(
        self, assignments: List[Dict], threshold: float
    ) -> List[Dict]:
        ...
        for assignment in assignments:
            ...
            if (
                ...
                and assignment["confidence"] > 0.3  # Not completely unassigned
                and assignment.get("identity", 0) is not None
                and assignment.get("identity", 0) < 90
            ):  # Low identity to known sequences
    ...
    def _calculate_alpha_diversity(self, assignments: List[Dict]) -> Dict[str, float]:
        ...
        for rank in ["genus", "species", "family"]:
            ...
            if taxa:
                ...
                chao1 = (
                    len(taxon_counts) + (singletons**2) / (2 * doubletons)
                    if doubletons > 0
                    else len(taxon_counts)
                )
                ...
                diversity_metrics[f"{rank}_evenness"] = (
                    shannon / np.log(len(taxon_counts)) if len(taxon_counts) > 1 else 0
                )
    ...
    def _prepare_beta_diversity_data(self, assignments: List[Dict]) -> Dict[str, Any]:
        ...
        return {
            ...
            "rare_genera": [
                genus for genus, count in genus_counts.items() if count < 5
            ],
    ...
    def _analyze_phylogenetic_patterns(self, assignments: List[Dict]) -> Dict[str, Any]:
        ...
        return {
            "kingdom_diversity": {
                ...
                "dominance": (
                    max(kingdom_counts.values()) / total_sequences
                    if kingdom_counts
                    else 0
                ),
            },
            "phylum_diversity": {
                ...
                "rare_phyla": [
                    phylum for phylum, count in phylum_counts.items() if count < 10
                ],
            },
    ...
    def _detect_contamination(self, kingdom_counts: Counter) -> Dict[str, Any]:
        ...
        contamination_indicators = {
            ...
            "unexpected_archaea": kingdom_counts.get("Archaea", 0)
            > total * 0.05,  # >5% archaea might be unexpected
            "low_eukaryote_ratio": kingdom_counts.get("Eukaryota", 0)
            < total * 0.1,  # <10% eukaryotes might be low for marine samples
            "contamination_score": (
                1 - max(kingdom_counts.values()) / total if total > 0 else 0
            ),
        }
