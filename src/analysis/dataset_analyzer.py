...
class DatasetAnalyzer:
    ...
    def __init__(self, config_path: Optional[str] = None, fast_mode: bool = False):
        ...
        self.advanced_taxonomic_analyzer = (
            AdvancedTaxonomicAnalyzer() if AdvancedTaxonomicAnalyzer else None
        )
        self.environmental_context_analyzer = (
            EnvironmentalContextAnalyzer() if EnvironmentalContextAnalyzer else None
        )
        self.enhanced_diversity_analyzer = (
            EnhancedDiversityAnalyzer() if EnhancedDiversityAnalyzer else None
        )
                ...
                try:
                    with opener(file_path, mode, encoding=encoding) as handle:
                        ...
                        if format_type == "fasta":
                            parse_format = (
                                "fasta-blast"  # Handles comments and is more robust
                            )
                        ...
                        for record in SeqIO.parse(handle, parse_format):
                            ...
                            if self.fast_mode:
                                if (
                                    count % 100000 == 0
                                ):  # Less frequent updates in fast mode
                            ...
                            with opener(file_path, mode, encoding=encoding) as handle:
                                for record in SeqIO.parse(handle, "fasta"):
                                    ...
                                    if count % 50000 == 0:
                                        print(f"   Loaded {count:,} sequences...")
                            print(
                                f"Successfully loaded with {encoding} encoding (standard FASTA)"
                            )
                            break
    ...
    def safe_sequence_str(self, seq_record):
        ...
        try:
            ...
            try:
                ...
                print(
                    f"   Warning: Could not extract sequence data, skipping problematic sequence"
                )
                return ""
        except Exception as e:
    ...
    def analyze_composition(
        self, sequences: List[SeqRecord], sequence_type: str = "auto"
    ) -> Dict[str, Any]:
        """
        ...
        if (
            self.fast_mode
            and len(sequences) > self.fast_mode_thresholds["large_dataset"]
        ):
            sample_size = self.fast_mode_thresholds["composition_sample"]
            print(
                f"   Fast mode: Analyzing {sample_size:,} representative sequences from {len(sequences):,} total..."
            )
            ...
            print(
                f"   Large dataset detected ({len(sequences):,} sequences). Using sampling for efficiency..."
            )
        ...
        print(
            f"   Processing {len(sample_sequences):,} sequences in {num_batches} batches (batch size: {batch_size})..."
        )
        ...
        for batch_idx in range(num_batches):
            ...
            if (
                num_batches > progress_interval
                and batch_idx % max(1, num_batches // progress_interval) == 0
            ):
                progress = (batch_idx + 1) / num_batches * 100
                print(
                    f"   Progress: {progress:.1f}% ({batch_idx + 1}/{num_batches} batches)"
                )
        ...
        composition = {
            char: count / total_chars for char, count in total_counter.items()
        }

        # Sort by frequency
        composition = dict(
            sorted(composition.items(), key=lambda x: x[1], reverse=True)
        )

        print(
            f"   Composition analysis complete. Found {len(total_counter)} unique characters."
        )
    ...
    def analyze_annotations(self, sequences: List[SeqRecord]) -> Dict[str, Any]:
        ...
        if (
            ...
            sample_sequences = sequences[::step][:sample_size]
            print(
                f"   Fast mode: Analyzing {len(sample_sequences):,} representative sequences from {len(sequences):,} total..."
            )
        else:
            sample_sequences = sequences
        ...
        for batch_idx in range(num_batches):
            ...
            for seq in batch:
                ...
                if not (
                    self.fast_mode
                    and len(sequences) > self.fast_mode_thresholds["very_large_dataset"]
                ):
        ...
        scaling_factor = (
            len(sequences) / len(sample_sequences)
            if len(sample_sequences) < len(sequences)
            else 1
        )
        ...
        if scaling_factor > 1:
            ...
            sequences_with_descriptions = int(
                sequences_with_descriptions * scaling_factor
            )
            sequences_with_features = int(sequences_with_features * scaling_factor)
            print(
                f"   Results scaled by factor {scaling_factor:.2f} for full dataset estimation"
            )

        print(
            f"   Annotation analysis complete. Processed {len(sample_sequences):,} sequences."
        )
    ...
    def analyze_quality(self, sequences: List[SeqRecord]) -> Dict[str, Any]:
        ...
        if (
            ...
            sample_sequences = sequences[::step][:sample_size]
            print(
                f"   Fast mode: Analyzing {len(sample_sequences):,} representative sequences for quality..."
            )
        else:
            sample_sequences = sequences
        ...
        for seq in sample_sequences:
            if (
                hasattr(seq, "letter_annotations")
                and "phred_quality" in seq.letter_annotations
            ):
        ...
        if scaling_factor > 1:
            quality_stats["sequences_with_quality"] = int(
                quality_stats["sequences_with_quality"] * scaling_factor
            )
            quality_stats["low_quality_sequences"] = int(
                quality_stats["low_quality_sequences"] * scaling_factor
            )
    ...
    def generate_report(
        ...
        if "composition" in analysis_results:
            ...
            lines.append(
                f"## 🧬 Sequence Composition ({comp['sequence_type'].upper()})"
            )
        ...
        if "annotations" in analysis_results:
            ...
            lines.append(
                f"- Sequences with organism info: {ann['sequences_with_organisms']:,}"
            )
            lines.append(
                f"- Sequences with descriptions: {ann['sequences_with_descriptions']:,}"
            )
            lines.append(
                f"- Sequences with features: {ann['sequences_with_features']:,}"
            )
            lines.append("")
        ...
        if "quality" in analysis_results:
            ...
            lines.append(
                f"- Sequences with quality scores: {qual['sequences_with_quality']:,}"
            )
            lines.append(
                f"- Low quality sequences (Q<20): {qual['low_quality_sequences']:,}"
            )
            if qual["sequences_with_quality"] > 0:
                ...
                lines.append(
                    f"- Quality range: {qual_dist['min']:.2f} - {qual_dist['max']:.2f}"
                )
            lines.append("")
        ...
        if "enhanced_diversity" in analysis_results:
            self._add_enhanced_diversity_section(
                lines, analysis_results["enhanced_diversity"]
            )

        # Advanced Taxonomic Analysis
        if "advanced_taxonomic" in analysis_results:
            self._add_advanced_taxonomic_section(
                lines, analysis_results["advanced_taxonomic"]
            )

        # Environmental Context Analysis
        if "environmental_context" in analysis_results:
            self._add_environmental_context_section(
                lines, analysis_results["environmental_context"]
            )
    ...
    def _add_enhanced_diversity_section(
        self, lines: List[str], diversity_data: Dict[str, Any]
    ) -> None:
        ...
        if "alpha_diversity" in diversity_data:
            ...
            lines.append(
                f"- Effective species number: {alpha.get('effective_species_number', 0):.2f}"
            )
        ...
        if "community_structure" in diversity_data:
            ...
            lines.append(
                f"- Dominant species (>5%): {community.get('dominant_species_count', 0)}"
            )
            lines.append(
                f"- Rare species (<1%): {community.get('rare_species_count', 0)}"
            )
            lines.append(
                f"- Evenness category: {community.get('community_evenness_category', 'unknown')}"
            )
            lines.append("")
        ...
        if "sampling_adequacy" in diversity_data:
            ...
            lines.append(
                f"- Completeness: {adequacy.get('sampling_completeness', 0):.2%}"
            )
            lines.append(
                f"- Adequacy level: {adequacy.get('adequacy_level', 'unknown')}"
            )
            if adequacy.get("additional_sampling_needed", False):
                lines.append("- ⚠️ Additional sampling recommended")
            lines.append("")

    def _add_advanced_taxonomic_section(
        self, lines: List[str], taxonomic_data: Dict[str, Any]
    ) -> None:
        ...
        if "confidence_assessment" in taxonomic_data:
            ...
            lines.append(
                f"- High confidence assignments: {confidence.get('high_confidence_count', 0)}"
            )
            lines.append(
                f"- Medium confidence assignments: {confidence.get('medium_confidence_count', 0)}"
            )
            lines.append(
                f"- Low confidence assignments: {confidence.get('low_confidence_count', 0)}"
            )
            lines.append(
                f"- Average confidence: {confidence.get('average_confidence', 0):.2%}"
            )
            lines.append("")
        ...
        if "novel_candidates" in taxonomic_data:
            ...
            if isinstance(novel_candidates, list):
                ...
                if total_assignments > 0:
                    ...
                    lines.append(
                        f"- Novel candidate percentage: {novel_percentage:.1f}%"
                    )
                else:
                ...
                if candidate_count > 0:
                    lines.append(
                        "✨ Novel species candidates detected - consider further investigation"
                    )
                    ...
                    if candidate_count > 0:
                        ...
                        for i, candidate in enumerate(novel_candidates[:5], 1):  # Top 5
                            ...
                            if isinstance(candidate, dict):
                                ...
                                potential_rank = candidate.get(
                                    "potential_rank", "unknown"
                                )
                                lines.append(
                                    f"{i}. {candidate.get('sequence_id', 'Unknown')} (novelty: {novelty_score:.3f}, rank: {potential_rank})"
                                )
                            else:
            ...
            # Fallback for unexpected data structure
                ...
                lines.append(f"- Potential novel species: {candidate_count}")
                lines.append(
                    f"- Novel candidate percentage: {novel_candidates.get('novel_percentage', 0):.1f}%"
                )
                if candidate_count > 0:
                    lines.append(
                        "✨ Novel species candidates detected - consider further investigation"
    ...
    def _add_environmental_context_section(
        self, lines: List[str], env_data: Dict[str, Any]
    ) -> None:
        ...
        if "habitat_classification" in env_data:
            ...
            lines.append(
                f"- Primary habitat: {habitat.get('primary_habitat', 'unknown')}"
            )
            lines.append(
                f"- Confidence: {habitat.get('classification_confidence', 0):.2%}"
            )
            lines.append(
                f"- Habitat complexity: {habitat.get('habitat_complexity', 'unknown')}"
            )
            lines.append("")
        ...
        if "ecological_indicators" in env_data:
            ...
            if indicators.get("indicator_species"):
                lines.append(
                    f"- Indicator species detected: {len(indicators['indicator_species'])}"
                )
            lines.append(
                f"- Ecosystem health: {indicators.get('ecosystem_health_score', 'unknown')}"
            )
            lines.append(
                f"- Pollution indicators: {indicators.get('pollution_indicators', 'none detected')}"
            )
            lines.append("")
        ...
        if "environmental_stress_indicators" in env_data:
            ...
            if stress.get("stress_indicators"):
                lines.append(
                    f"- Stress indicators detected: {len(stress['stress_indicators'])}"
                )
            lines.append("")
    ...
    def analyze_dataset(
        ...
        if self.advanced_taxonomic_analyzer:
            try:
                ...
                results["advanced_taxonomic"] = (
                    self.advanced_taxonomic_analyzer.analyze_taxonomic_composition(
                        sequences
                    )
                )
        ...
        if self.enhanced_diversity_analyzer:
            try:
                ...
                taxonomic_data = self._prepare_taxonomic_data(
                    results.get("annotations", {})
                )
                results["enhanced_diversity"] = (
                    self.enhanced_diversity_analyzer.analyze_comprehensive_diversity(
                        taxonomic_data
                    )
                )
        ...
        if self.environmental_context_analyzer:
            try:
                ...
                results["environmental_context"] = (
                    self.environmental_context_analyzer.analyze_environmental_context(
    ...
    def _prepare_taxonomic_data(self, annotations: Dict[str, Any]) -> Dict[str, Any]:
        ...
        taxonomic_data = {
            "taxonomic_distribution": {
                "genus": {"counts": {}, "unique_taxa": 0},
                "species": {"counts": {}, "unique_taxa": 0},
            }
        }
        ...
        if "organism_distribution" in annotations:
            ...
            genus_counts = {
                k: v for k, v in genus_counts.items() if v is not None and v > 0
            }
            species_counts = {
                k: v for k, v in species_counts.items() if v is not None and v > 0
            }

            taxonomic_data["taxonomic_distribution"]["genus"]["counts"] = genus_counts
            taxonomic_data["taxonomic_distribution"]["genus"]["unique_taxa"] = len(
                genus_counts
            )
            taxonomic_data["taxonomic_distribution"]["species"][
                "counts"
            ] = species_counts
            taxonomic_data["taxonomic_distribution"]["species"]["unique_taxa"] = len(
                species_counts
            )
...
Examples:
  ...
  python dataset_analyzer.py swissprot.gz analysis.txt --format fasta --max 10000
    ...
    parser.add_argument(
        "--format", help="Force specific format (fasta, fastq, swiss, etc.)"
    )
    parser.add_argument(
        "--max", type=int, help="Maximum number of sequences to analyze"
    )
