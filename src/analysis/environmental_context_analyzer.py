...
class EnvironmentalContextAnalyzer:
    ...
    def __init__(self):
        ...
        self.habitat_classifiers = {
            ...
            "extreme": {
                "temperature": (60, 100),
                "ph": (0, 3),
            },  # Hot springs, acidic environments
        }

    def analyze_environmental_context(
        self, environmental_data: Dict[str, Any], taxonomic_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
    ...
    def _classify_habitat(self, env_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        primary_habitat = (
            max(habitat_scores.keys(), key=lambda k: habitat_scores[k])
            if habitat_scores
            else "unknown"
        )
        ...
        return {
            ...
            "habitat_complexity": self._assess_habitat_complexity(
                env_data, primary_habitat
            ),
            "habitat_description": self._get_habitat_description(
                primary_habitat, env_data
            ),
        }
    ...
    def _identify_ecological_indicators(
        self, env_data: Dict[str, Any], tax_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        ph = env_data.get("ph")
        depth = env_data.get("depth")
        if ph is not None and ph > 7.5 and depth is not None and depth > 100:
        ...
        indicators["ecosystem_health_score"] = (
            np.mean(health_factors) if health_factors else 0.5
        )

        return indicators

    def _analyze_habitat_specificity(
        self, env_data: Dict[str, Any], tax_data: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _detect_environmental_stress(
        self, env_data: Dict[str, Any], tax_data: Dict[str, Any]
    ) -> Dict[str, Any]:
            ...
            stress_indicators["stress_response_taxa"].append(
                {
                    ...
                    "stress_level": (
                        "high" if salinity > 80 or salinity < 0.01 else "moderate"
                    ),
                }
            )
        ...
        if collection_date:
            ...
            seasonal_insights = {
                "winter": {
                    ...
                    "dominant_groups": ["Cold-adapted bacteria"],
                },
                "spring": {
                    "expected_diversity": "increasing",
                    "dominant_groups": ["Bloom-forming algae"],
                },
                "summer": {
                    "expected_diversity": "high",
                    "dominant_groups": ["Thermophilic bacteria"],
                },
                "autumn": {
                    "expected_diversity": "moderate",
                    "dominant_groups": ["Decomposer bacteria"],
                },
            }
    ...
    def _analyze_depth_stratification(
        self, env_data: Dict[str, Any], tax_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if depth < 10:
            ...
            stratification["pressure_adaptations"] = [
                "Piezophilic_bacteria",
                "Barophiles",
            ]
            stratification["depth_specific_taxa"] = [
                "Deep_sea_specialists",
                "Pressure_adapted_archaea",
            ]

        return stratification

    def _analyze_biogeochemical_context(
        self, env_data: Dict[str, Any], tax_data: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _generate_conservation_insights(
        self, env_data: Dict[str, Any], tax_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if temp is not None and (temp > 40 or temp < 5):
            ...
            conservation["monitoring_recommendations"].append(
                "climate_change_monitoring"
            )

        return conservation

    def _generate_sampling_recommendations(
        self, env_data: Dict[str, Any], analysis_results: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if (
            analysis_results.get("seasonal_patterns", {}).get("sampling_season")
            == "winter"
        ):
    ...
    def _get_temperature_implications(self, temp: float) -> List[str]:
        ...
        if temp < 5:
            implications.extend(
                ["psychrophilic_adaptations", "slow_metabolism", "cold_shock_proteins"]
            )
        elif temp > 30:
            implications.extend(
                ["thermophilic_adaptations", "heat_shock_response", "protein_stability"]
            )
        return implications
    ...
    def _predict_depth_adaptations(self, depth: float) -> List[str]:
        ...
        if depth > 1000:
            adaptations.extend(
                ["piezophilic_adaptations", "specialized_membrane_composition"]
            )
        return adaptations
    ...
    def _get_ph_implications(self, ph: float) -> List[str]:
        ...
        if ph < 5:
            implications.extend(
                ["acid_tolerance", "acid_stress_response", "specialized_pH_homeostasis"]
            )
        elif ph > 9:
            implications.extend(
                ["alkaline_tolerance", "base_stress_response", "alkaline_adaptations"]
            )
        return implications
    ...
    def _get_salinity_adaptations(self, salinity: float) -> List[str]:
        ...
        if salinity > 30:
            adaptations.extend(
                ["osmoregulation", "compatible_solutes", "salt_tolerance"]
            )
        elif salinity < 5:
