...
class EnhancedDiversityAnalyzer:
    ...
    def __init__(self):
        ...
        self.diversity_metrics = [
            ...
            "menhinick",
            "margalef",
            "effective_species_number",
        ]

    def analyze_comprehensive_diversity(
        self,
        taxonomic_data: Dict[str, Any],
        abundance_data: Optional[Dict[str, int]] = None,
    ) -> Dict[str, Any]:
        """
        ...
        results["alpha_diversity"] = self._calculate_alpha_diversity_comprehensive(
            abundance_data
        )

        # Rarefaction analysis
        results["rarefaction_analysis"] = self._perform_rarefaction_analysis(
            abundance_data
        )

        # Community structure analysis
        results["community_structure"] = self._analyze_community_structure(
            abundance_data
        )
        ...
        results["diversity_comparisons"] = self._generate_diversity_comparisons(
            results["alpha_diversity"]
        )

        # Functional diversity estimation
        results["functional_diversity"] = self._estimate_functional_diversity(
            taxonomic_data, abundance_data
        )

        # Phylogenetic diversity estimation
        results["phylogenetic_diversity"] = self._estimate_phylogenetic_diversity(
            taxonomic_data, abundance_data
        )
        ...
        results["sampling_adequacy"] = self._assess_sampling_adequacy(
            abundance_data, results["rarefaction_analysis"]
        )

        return results

    def _extract_abundance_from_taxonomic_data(
        self, taxonomic_data: Dict[str, Any]
    ) -> Dict[str, int]:
        ...
        if "taxonomic_distribution" in taxonomic_data:
            ...
            if "counts" in genus_data:
                ...
                abundance_data = {
                    k: int(v) for k, v in raw_counts.items() if v is not None and v > 0
                }
        ...
        if not abundance_data:
            ...
            abundances = (abundances / abundances.sum() * 1000).astype(
                int
            )  # Scale to reasonable total

            abundance_data = {
                f"Species_{i+1}": int(abundance)
    ...
    def _calculate_alpha_diversity_comprehensive(
        self, abundance_data: Dict[str, int]
    ) -> Dict[str, float]:
        ...
        metrics["inverse_simpson"] = (
            1 / sum(p**2 for p in proportions)
            if sum(p**2 for p in proportions) > 0
            else 0
        )

        # Pielou's evenness
        metrics["evenness"] = (
            metrics["shannon"] / np.log(n_species) if n_species > 1 else 0
        )
        ...
        # Fisher's alpha
        metrics["fisher_alpha"] = self._calculate_fisher_alpha(
            abundances, total_individuals
        )

        # Berger-Parker index
        metrics["berger_parker"] = (
            max(abundances) / total_individuals if total_individuals > 0 else 0
        )

        # McIntosh index
        metrics["mcintosh"] = (
            np.sqrt(sum(n**2 for n in abundances)) / total_individuals
        ...
        # Menhinick's index
        metrics["menhinick"] = (
            n_species / np.sqrt(total_individuals) if total_individuals > 0 else 0
        )

        # Margalef's index
        metrics["margalef"] = (
            (n_species - 1) / np.log(total_individuals) if total_individuals > 1 else 0
        )
    ...
    def _calculate_chao1(self, abundances: List[int]) -> float:
        ...
        valid_abundances = [
            count for count in abundances if count is not None and count > 0
        ]
        ...
        if doubletons > 0:
            ...
            chao1 = (
                observed_species + singletons * (singletons - 1) / 2
                if singletons > 0
    ...
    def _calculate_ace(self, abundances: List[int]) -> float:
        ...
        abundant_species = [
            count for count in valid_abundances if count > rare_threshold
        ]
        ...
        if c_ace > 0:
            gamma_ace = max(
                ...
                (s_rare / c_ace)
                * sum(
                    (i * (i - 1) * sum(1 for count in rare_species if count == i))
    ...
    def _calculate_fisher_alpha(
        self, abundances: List[int], total_individuals: int
    ) -> float:
    ...
    def _calculate_brillouin(
        ...
        try:
            log_factorial_total = sum(
                np.log(i) for i in range(1, total_individuals + 1)
            )
            log_factorial_abundances = sum(
                sum(np.log(i) for i in range(1, count + 1))
                for count in valid_abundances
                if count > 0
            )

            brillouin = (
                log_factorial_total - log_factorial_abundances
            ) / total_individuals
            return brillouin
    ...
    def _perform_rarefaction_analysis(
        self, abundance_data: Dict[str, int]
    ) -> Dict[str, Any]:
        """Perform rarefaction analysis to assess sampling completeness."""
        # Filter out None values and ensure positive integers
        valid_abundances = [
            v for v in abundance_data.values() if v is not None and v > 0
        ]

        if not valid_abundances:
            return {
                "rarefaction_curve": [],
                "extrapolated_richness": 0,
                "sampling_completeness": 0,
            }
        ...
        sample_sizes = np.linspace(
            1, total_individuals, min(50, total_individuals)
        ).astype(int)
        rarefaction_curve = []

        for sample_size in sample_sizes:
            expected_species = self._calculate_rarefied_richness(
                valid_abundances, sample_size
            )
            rarefaction_curve.append(
                {"sample_size": int(sample_size), "expected_species": expected_species}
            )
        ...
        return {
            ...
            "asymptote_approach": (
                "approaching" if sampling_completeness > 0.8 else "steep"
            ),
        }

    def _calculate_rarefied_richness(
        self, abundances: List[int], sample_size: int
    ) -> float:
        ...
        for abundance in abundances:
            ...
            prob_present = 1 - (
                comb(total_individuals - abundance, sample_size, exact=False)
    ...
    def _analyze_community_structure(
        self, abundance_data: Dict[str, int]
    ) -> Dict[str, Any]:
        ...
        percentages = [
            (abundance / total_individuals) * 100 for abundance in sorted_abundances
        ]

        # Identify community structure patterns
        structure = {
            "rank_abundance_distribution": list(
                zip(ranks, sorted_abundances, percentages)
            ),
            "dominant_species_count": sum(
                1 for p in percentages if p > 5
            ),  # Species with >5% abundance
            "rare_species_count": sum(
                1 for p in percentages if p < 1
            ),  # Species with <1% abundance
            "core_species_count": sum(
                1 for p in percentages if 1 <= p <= 5
            ),  # Species with 1-5% abundance
            ...
            "species_accumulation": self._calculate_species_accumulation(
                sorted_abundances
            ),
        }
    ...
    def _analyze_dominance_pattern(self, percentages: List[float]) -> Dict[str, Any]:
        ...
        return {
            ...
            "dominance_ratio": (
                percentages[0] / percentages[-1] if len(percentages) > 1 else 1
            ),
            "top_5_species_percentage": (
                sum(percentages[:5]) if len(percentages) >= 5 else sum(percentages)
            ),
        }

    def _calculate_species_accumulation(
        self, sorted_abundances: List[int]
    ) -> List[Dict[str, int]]:
    ...
    def _analyze_rare_species(self, abundance_data: Dict[str, int]) -> Dict[str, Any]:
        ...
        rare_count = sum(
            1 for count in valid_abundances if count <= 5
        )  # ≤5 individuals
        very_rare_count = sum(
            1 for count in valid_abundances if count <= 2
        )  # ≤2 individuals
        ...
        return {
            ...
            "singleton_percentage": (
                (singleton_count / len(valid_abundances)) * 100
                if valid_abundances
                else 0
            ),
            "rare_species_percentage": (
                (rare_count / len(valid_abundances)) * 100 if valid_abundances else 0
            ),
            "rare_species_abundance_percentage": (
                sum(count for count in valid_abundances if count <= 5)
                / total_individuals
            )
            * 100,
            "rarity_assessment": self._assess_rarity_level(
                singleton_count, len(valid_abundances)
            ),
        }
    ...
    def _analyze_dominance_patterns(
        self, abundance_data: Dict[str, int]
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "dominance_concentration": {
                ...
                "top_3_species": (
                    (sum(sorted_abundances[:3]) / total_individuals) * 100
                    if len(sorted_abundances) >= 3
                    else 100
                ),
                "top_10_species": (
                    (sum(sorted_abundances[:10]) / total_individuals) * 100
                    if len(sorted_abundances) >= 10
                    else 100
                ),
            },
    ...
    def _generate_diversity_comparisons(
        self, alpha_diversity: Dict[str, float]
    ) -> Dict[str, Any]:
    ...
    def _estimate_functional_diversity(
        self, taxonomic_data: Dict[str, Any], abundance_data: Dict[str, int]
    ) -> Dict[str, Any]:
        ...
        functional_proportions = (
            {
                group: count / total_abundance
                for group, count in functional_abundance.items()
            }
        ...
        functional_shannon = -sum(
            p * np.log(p) for p in functional_proportions.values() if p > 0
        )
        ...
        return {
            ...
            "functional_richness": len(
                [g for g, a in functional_abundance.items() if a > 0]
            ),
            "functional_evenness": (
                functional_shannon / np.log(len(functional_abundance))
                if len(functional_abundance) > 1
                else 0
            ),
        }

    def _estimate_phylogenetic_diversity(
        self, taxonomic_data: Dict[str, Any], abundance_data: Dict[str, int]
    ) -> Dict[str, Any]:
        ...
        total_ranks = sum(
            phylo_diversity[f"{rank}_diversity"]
            for rank in ["kingdom", "phylum", "class", "order", "family", "genus"]
        )
        phylo_diversity["phylogenetic_dispersion"] = total_ranks

        # Taxonomic breadth
        phylo_diversity["taxonomic_breadth"] = len(
            [v for v in phylo_diversity.values() if isinstance(v, int) and v > 0]
        )
    ...
    def _assess_sampling_adequacy(
        self, abundance_data: Dict[str, int], rarefaction_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Assess sampling adequacy and provide recommendations."""
        observed_richness = len(abundance_data)
        estimated_richness = rarefaction_data.get(
            "extrapolated_richness", observed_richness
        )
        ...
        adequacy = {
            ...
            "recommendations": self._generate_sampling_recommendations(
                completeness, observed_richness
            ),
        }
    ...
    def _generate_sampling_recommendations(
        self, completeness: float, observed_richness: int
    ) -> List[str]:
        ...
        if completeness < 0.8:
            ...
            recommendations.append(
                f"Collect approximately {additional_samples} additional samples"
            )
