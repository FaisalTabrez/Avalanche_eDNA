...
def render_buffer_composition(buffer_stats: Dict[str, Any]) -> None:
    ...
    with col3:
        st.metric(
            "Recent Buffer", f"{recent_size:,}", help="Most recently seen samples"
        )
...
def render_memory_usage_gauge(memory_mb: float, budget_gb: float) -> None:
    ...
    with col3:
        status = (
            "✓ OK" if usage_pct < 80 else "⚠ High" if usage_pct < 95 else "🔴 Critical"
        )
...
def render_training_metrics_evolution(training_history: Dict[str, Any]) -> None:
    ...
    fig.update_layout(
        ...
        yaxis2=dict(
            title="Accuracy (%)", side="right", overlaying="y", color="#2ecc71"
        ),
        hovermode="x unified",
        template="plotly_white",
...
def render_cluster_size_distribution(cluster_labels: np.ndarray) -> None:
    ...
    fig.update_layout(
        xaxis={"type": "category"}, showlegend=False, template="plotly_white"
    )
...
def render_dynamic_scaling_summary(pipeline) -> None:
    ...
    tab1, tab2, tab3, tab4 = st.tabs(
        ["📊 Overview", "📅 Adaptations", "💾 Memory", "⚙️ Configuration"]
    )
