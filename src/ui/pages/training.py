...
def render_training_interface():
    ...
    if create_sra_data_source_selector:
        ...
        data_source = st.radio(
            "Data Source", ["Upload New File", "Select Existing Dataset"]
        )
        ...
        if data_source == "Upload New File":
            ...
            if datasets_dir.exists():
                files = list(datasets_dir.glob("*.fasta")) + list(
                    datasets_dir.glob("*.fa")
                )
                if files:
    ...
    st.markdown(
        "Upload labels for supervised training. Leave empty for unsupervised/contrastive learning."
    )
    labels_file = st.file_uploader("Upload Labels", type=["csv", "txt"])
    labels_path = None
    if labels_file:
        with tempfile.NamedTemporaryFile(
            delete=False, suffix=Path(labels_file.name).suffix
        ) as tmp:
    ...
    with col1:
        ...
        epochs = st.number_input(
            "Training Epochs", min_value=1, max_value=1000, value=50
        )
        batch_size = st.number_input("Batch Size", min_value=2, max_value=512, value=32)
        learning_rate = st.number_input(
            "Learning Rate", min_value=1e-6, max_value=1.0, value=1e-4, format="%.6f"
        )

    with col2:
        st.markdown("#### 🔧 Model Parameters")
        embedding_dim = st.number_input(
            "Embedding Dimension", min_value=32, max_value=2048, value=256
        )

        if model_type == "Contrastive Learning":
            projection_dim = st.number_input(
                "Projection Dimension", min_value=32, max_value=1024, value=64
            )  # OPTIMIZED
            temperature = st.number_input(
                "Temperature", min_value=0.01, max_value=1.0, value=0.1
            )
        else:
            projection_dim = None
            temperature = None
...
def render_scaling_config():
    ...
    config_mode = st.radio(
        "Configuration Mode", ["🎯 Use Preset", "🔧 Custom Settings"], horizontal=True
    )
    ...
    if config_mode == "🎯 Use Preset":
        ...
        with st.expander("📊 Preset Details"):
            ...
            with col1:
                ...
                st.metric(
                    "Target Accuracy", f"{preset_cfg['target_accuracy']*100:.0f}%"
                )
            with col2:
        ...
        with st.expander("🔬 Advanced Options"):
            ...
            with col1:
                batch_size_scaling = st.number_input(
                    "Batch Size", min_value=8, max_value=512, value=32
                )
                dropout_rate = st.slider(
                    "Dropout Rate", min_value=0.0, max_value=0.5, value=0.1, step=0.05
                )

            with col2:
                ewc_lambda = st.number_input(
                    "EWC Lambda", min_value=0.0, max_value=100.0, value=1.0, step=0.1
                )
                use_lora = st.checkbox(
                    "Use LoRA",
                    value=False,
                    help="Low-Rank Adaptation for efficient fine-tuning",
                )
    ...
    total_samples = (
        config.exemplars_per_cluster * 50  # Assume 50 clusters for estimate
        + config.uncertainty_buffer_size
        + config.recent_buffer_size
    )
    memory_mb = (
        total_samples * 768 * 4 / (1024**2)
    )  # 768-dim embedding, 4 bytes per float

    st.info(
        f"💾 **Estimated Memory Usage:** {memory_mb:.1f} MB ({total_samples:,} samples in buffer)"
    )
...
def train_model_ui(
    ...
    try:
        ...
        # Load data
        from scripts.train_model import create_model, load_labels, load_sequences
        ...
        config_dict = {
            ...
            "training": {"projection_dim": projection_dim, "temperature": temperature},
        }
        ...
        if enable_scaling and scaling_config:
            ...
            status_container.info(
                "⚡ Dynamic scaling enabled - training with continual learning"
            )
            ...
            history = trainer.train_with_dynamic_scaling(
                ...
                max_length=512,
                replay_ratio=(
                    scaling_config.replay_ratio
                    if hasattr(scaling_config, "replay_ratio")
                    else 0.3
                ),
                validation_split=0.2,
            )
            ...
            for epoch in range(epochs):
                ...
                if model_type == "autoencoder":
                    epoch_history = trainer.train_autoencoder(
                        train_loader, val_loader, epochs=1, learning_rate=learning_rate
                    )
                else:
                    epoch_history = trainer.train_contrastive(
                        train_loader, val_loader, epochs=1, learning_rate=learning_rate
                    )
        ...
        if enable_scaling and scaling_config:
            ...
            if "buffer_size" in history:
                ...
                metadata["final_exemplars_per_cluster"] = history[
        ...
        with col3:
            total_epochs = (
                ...
                else f"{len(history['train_loss'])} tasks"
            )
        ...
        with st.expander(f"🤖 {model_dir.name}", expanded=False):
            ...
            with col1:
                ...
                if meta_path.exists():
                    ...
                    with info_cols[0]:
                        st.metric(
                            "Architecture", meta.get("model_type", "Unknown").title()
                        )
                    with info_cols[1]:
                    ...
                    with loss_cols[0]:
                        st.metric(
                            "Final Train Loss", f"{meta.get('final_train_loss', 0):.4f}"
                        )
                    with loss_cols[1]:
                        st.metric(
                            "Final Val Loss", f"{meta.get('final_val_loss', 0):.4f}"
                        )
                    with loss_cols[2]:
                        st.metric(
                            "Learning Rate", f"{meta.get('learning_rate', 0):.6f}"
                        )
                    ...
                    if meta.get("dynamic_scaling_enabled"):
                        ...
                        if scaling_config:
                            ...
                            with scale_cols[0]:
                                st.metric(
                                    "Exemplars/Cluster",
                                    scaling_config.get("exemplars_per_cluster", "N/A"),
                                )
                            with scale_cols[1]:
                                st.metric(
                                    "Uncertainty Buffer",
                                    scaling_config.get(
                                        "uncertainty_buffer_size", "N/A"
                                    ),
                                )
                            with scale_cols[2]:
                                st.metric(
                                    "Recent Buffer",
                                    scaling_config.get("recent_buffer_size", "N/A"),
                                )
                            with scale_cols[3]:
                                st.metric(
                                    "Replay Ratio",
                                    f"{scaling_config.get('replay_ratio', 0):.2f}",
                                )
                            ...
                            if meta.get("final_clusters"):
                                ...
                                with final_cols[0]:
                                    st.metric(
                                        "Final Clusters", meta.get("final_clusters")
                                    )
                                with final_cols[1]:
                                    st.metric(
                                        "Final Buffer Size",
                                        f"{meta.get('final_buffer_size', 0):,}",
                                    )
                                with final_cols[2]:
                                    memory_mb = (
                                        meta.get("final_buffer_size", 0)
                                        * 768
                                        * 4
                                        / (1024**2)
                                    )
                        ...
                        st.info(
                            "ℹ️ Trained without dynamic scaling (standard training)"
                        )
                    ...
                    if history_path.exists():
                        ...
                        fig = px.line(
                            ...
                            x=(
                                ...
                                else history_df.index + 1
                            ),
                            y=(
                                ["train_loss", "val_loss"]
                                if "train_loss" in history_df.columns
                                else ["Train Loss", "Val Loss"]
                            ),
                            title="Loss Curve",
                            labels={"value": "Loss", "variable": "Metric"},
                        )
                        ...
                        if (
                            "clusters" in history_df.columns
                            and history_df["clusters"].notna().any()
                        ):
                            fig2 = px.line(
                                history_df,
            ...
            with col2:
                ...
                # Model path
                st.text_input(
                    "Path", str(model_dir), disabled=True, label_visibility="collapsed"
                )

                # Delete button
                if st.button(
                    "🗑️ Delete Model",
                    key=f"del_{model_dir.name}",
                    type="secondary",
