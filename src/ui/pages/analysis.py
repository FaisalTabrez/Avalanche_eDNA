    ...
    try:
        ...
        if data_source_type == "Upload File":
            ...
            if SRAIntegrationUI:
                ...
                if sra_ui.sra_toolkit_available:
                    ...
                    if accession_input and st.button(
                        "Download SRA Dataset", type="primary"
                    ):
        ...
        if uploaded_file is not None:
            try:
                ...
                if file_size_mb > 10240:  # 10GB limit
                    st.error(
                        "File size exceeds 10GB limit. Please use a smaller file or contact support for processing very large datasets."
                    )
                    file_valid = False
                elif file_size_mb > 1024:  # Warn for files over 1GB
                    st.warning(
                        f"Large file detected ({file_size_mb:.0f} MB). Upload may take longer than usual. Please be patient."
                    )

            except Exception as e:
                st.error(f"Error reading file: {str(e)}")
                st.error(
                    "This might be due to a network timeout for large files. Please try with a smaller file or check your connection."
                )
                file_valid = False
        ...
        with col1:
            dataset_name = st.text_input(
                "Dataset Name", value="My Dataset", help="Custom name for your analysis"
            )
        ...
        with st.expander("Advanced Options"):
            ...
            with col1:
                ...
                fast_mode = st.checkbox(
                    "Fast Mode (Recommended for >10K sequences)",
                    value=True,
        ...
        if fast_mode:
            st.info(
                "Fast Mode enabled: Analysis will use intelligent sampling for datasets >10K sequences, reducing time by 60-80%"
            )
...
def run_analysis(
    ...
    safe_name = "".join(
        ...
        for c in (dataset_name or "dataset")
    )
    ...
    try:
        ...
        # Prepare parameters
        format_type = (
            None if format_override == "Auto-detect" else format_override.lower()
        )
        ...
        if (
            uploaded_file and len(uploaded_file.getvalue()) > 100 * 1024 * 1024
        ):  # > 100MB
            st.warning(
                "Large file detected. Analysis may take several minutes. Please be patient..."
            )
            if len(uploaded_file.getvalue()) > 200 * 1024 * 1024:  # > 200MB
                st.warning(
                    "For files > 200MB, consider using a smaller subset or the command-line tool for better performance."
                )
        ...
        if not results or "basic_stats" not in results:
            raise ValueError(
                "Analysis did not complete successfully or returned incomplete results"
            )
...
def display_results(results, output_path, enable_visualization):
    ...
    with col3:
        st.metric("Sequence Type", comp.get("sequence_type", "Unknown").upper())

    with col4:
        st.metric("Processing Time", f"{proc.get('total_time', 0):.2f}s")

    # Detailed results tabs
    tab1, tab2, tab3, tab4, tab5 = st.tabs(
        ["Statistics", "Composition", "Annotations", "Biodiversity", "Report"]
    )
...
def show_composition_tab(results, enable_visualization):
    ...
    if comp.get("composition"):
        ...
        with col1:
            st.markdown(
                f"### {comp.get('sequence_type', 'Unknown').upper()} Composition"
            )
...
def show_annotations_tab(results):
    ...
    with col1:
        st.metric("With Organisms", f"{ann.get('sequences_with_organisms', 0):,}")

    with col2:
        st.metric("With Descriptions", f"{ann.get('sequences_with_descriptions', 0):,}")

    with col3:
        st.metric("With Features", f"{ann.get('sequences_with_features', 0):,}")
...
def show_biodiversity_tab(results):
    ...
    if div:
        ...
        with col1:
            st.metric("Species Richness", f"{div.get('species_richness', 0):,}")

            st.metric("Shannon Diversity", f"{div.get('shannon_diversity', 0):.4f}")

        with col2:
            st.metric("Simpson Diversity", f"{div.get('simpson_diversity', 0):.4f}")

            st.metric("Evenness", f"{div.get('evenness', 0):.4f}")
