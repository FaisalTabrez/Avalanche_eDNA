...
def render_presets():
    ...
    # Memory estimate
    total_samples = (
        config.exemplars_per_cluster * preset_cfg["n_clusters"]
    ...
    with st.expander("🎛️ Fine-Tune Parameters"):
        ...
        with col2:
            ...
            batch_size = st.number_input(
                "Batch Size", min_value=8, max_value=256, value=config.batch_size
            )
    ...
    if uploaded_file:
        try:
            ...
            if st.button("✓ Use This Configuration"):
                ...
                st.success(
                    "Configuration saved to session! Use it in the Analysis page."
                )
...
def display_config_summary(config: ScalingConfig, n_clusters: int):
    ...
    # Memory calculation
    total_samples = (
        config.exemplars_per_cluster * n_clusters
        + config.uncertainty_buffer_size
        + config.recent_buffer_size
    )
    memory_mb = total_samples * 768 * 4 / (1024**2)
