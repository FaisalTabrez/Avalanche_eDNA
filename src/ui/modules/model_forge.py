...
def render():
    ...
    st.markdown(
        "Train, fine-tune, and scale your eDNA models with **DNABERT-2 on CPU**."
    )
...
def render_training_tab():
    ...
    if not MODELS_AVAILABLE:
        st.error(
            "❌ Model training components not available. Please check installation."
        )
        return
    ...
    with col1:
        ...
        data_source = st.radio(
            "Source", ["Upload File", "Select Dataset"], label_visibility="collapsed"
        )
            ...
            if datasets_dir.exists():
                ...
                if files:
                    selected = st.selectbox(
                        "Dataset", files, format_func=lambda x: x.name
                    )
        ...
        if enable_scaling and SCALING_AVAILABLE:
            memory_budget = st.number_input(
                "Memory Budget (GB)", value=2.0, min_value=0.5, max_value=32.0
            )
...
def render_scaling_tab():
    ...
    with col1:
        ...
        preset = st.selectbox(
            ...
            [
                "Small (5-20 clusters)",
                "Medium (20-100 clusters)",
                "Large (100-500 clusters)",
                "Custom",
            ],
        )
        ...
        uncertainty_buffer = st.number_input(
            "Uncertainty Buffer Size", 100, 10000, 1000
        )
...
def train_dnabert2_cpu(
    ...
    try:
        ...
        config_dict = {
            ...
            "training": {"projection_dim": 128, "temperature": 0.1},
        }
