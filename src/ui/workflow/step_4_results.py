...
def render_taxonomy_tab(results: Optional[Dict[str, Any]]):
    ...
    st.info(
        "23 potentially novel taxonomic groups identified requiring further investigation"
    )
...
def render_quality_tab(results: Optional[Dict[str, Any]]):
    ...
    demo_quality = pd.DataFrame({"Quality Score": np.random.normal(8.5, 0.85, 1000)})
    ...
    demo_training = pd.DataFrame(
        {
            "Epoch": epochs,
            "Training Loss": [
                0.8 - (i * 0.013) + np.random.normal(0, 0.02) for i in epochs
            ],
            "Validation Accuracy": [
                60 + (i * 0.7) + np.random.normal(0, 1.5) for i in epochs
            ],
        }
    )
