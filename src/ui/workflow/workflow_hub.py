...
def render_configure_step():
    ...
    if st.session_state.workflow_dataset:
        st.success(
            f"✓ Dataset: {st.session_state.workflow_dataset.get('name', 'Unknown')}"
        )
...
def render_task_panel():
    ...
    with col2:
        if st.button(
            "▲ Collapse" if st.session_state.task_panel_expanded else "▼ Expand",
            key="toggle_task_panel",
        ):
            st.session_state.task_panel_expanded = (
                not st.session_state.task_panel_expanded
            )
            st.rerun()
