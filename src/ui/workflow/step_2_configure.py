...
def render_configuration():
    ...
    if dataset:
        ...
        with col1:
            st.info(f"📂 Dataset: **{dataset.get('name')}** ({dataset.get('size_mb', 0):.2f} MB)")
        with col2:
    ...
    selected_preset = list(PRESETS.keys())[preset_options.index(selected_preset_display)]
        ...
        if model_mode == 'train_new':
            ...
            with col2:
                ...
                architecture = st.selectbox(
                    "Architecture",
                    options=['contrastive', 'supervised'],
                    index=0
                )
