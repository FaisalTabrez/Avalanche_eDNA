...
def render_dataset_selection():
    ...
    # Dataset source tabs
    tab1, tab2, tab3 = st.tabs(["📤 Upload File", "💾 Existing Datasets", "🌐 Download from SRA"])
    ...
    if st.session_state.workflow_dataset:
        st.success(f"✓ Selected: {st.session_state.workflow_dataset.get('name', 'Unknown')}")
...
def render_upload_tab():
    ...
    if uploaded_file is not None:
        ...
        if file_size_mb > 10240:  # 10GB limit
            ...
            st.warning(f"⚠️ Large file ({file_size_mb:.0f} MB). Upload may take longer.")
...
def render_existing_tab():
    ...
    sequence_files = [
        f for f in files 
        if f.suffix.lower() in ['.fasta', '.fa', '.fas', '.fastq', '.fq', '.swiss', '.gb', '.gbk', '.embl', '.em', '.gz']
    ]
    ...
    # Display as table
    dataset_data = []
    for file_path in sorted(sequence_files, key=lambda x: x.stat().st_mtime, reverse=True):
