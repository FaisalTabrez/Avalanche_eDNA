...
class ScalingConfig:
    ...
    def auto_scale(cls, 
                   ...
                   target_accuracy: float = 0.80) -> 'ScalingConfig':
        """
        ...
        hierarchy_levels = cls._calculate_hierarchy_levels(n_clusters) if use_hierarchical else None
    ...
    @staticmethod
    def _calculate_exemplars_per_cluster(n_clusters: int, dataset_size: int, 
                                         memory_gb: float, target_coverage: float) -> int:
    ...
    @staticmethod
    def _calculate_uncertainty_buffer(n_clusters: int, dataset_size: int,
                                     memory_gb: float, exemplars_per_cluster: int) -> int:
        """Calculate uncertainty buffer size."""
        # 30% of memory budget
    ...
    @staticmethod
    def _calculate_recent_buffer(n_clusters: int, dataset_size: int,
                                memory_gb: float, exemplars_per_cluster: int) -> int:
        """Calculate recent buffer size."""
        # 30% of memory budget
    ...
    def get_memory_estimate(self) -> Dict[str, float]:
        ...
        exemplar_mb = (self.exemplars_per_cluster * self.n_clusters * bytes_per_embedding) / (1024**2)
        uncertainty_mb = (self.uncertainty_buffer_size * bytes_per_embedding) / (1024**2)
...
class DynamicHybridBuffer:
    ...
    def __init__(self, 
                 initial_config: Optional[ScalingConfig] = None,
    ...
    def add_cluster(self, cluster_id: int, samples: np.ndarray,
                   cluster_labels: np.ndarray, logits: Optional[torch.Tensor] = None):
        """
    ...
    def _get_config_changes(self, old: ScalingConfig, new: ScalingConfig) -> List[str]:
        ...
        if old.hidden_dims != new.hidden_dims:
            changes.append(
                f"Architecture: {old.hidden_dims} → {new.hidden_dims}"
            )
        ...
        if old.use_lora != new.use_lora:
            changes.append(
                f"LoRA adapters: {old.use_lora} → {new.use_lora}"
            )
        
        return changes
    
    def _transfer_buffer_data(self, old_buffer: HybridMemoryBuffer, 
                             new_buffer: HybridMemoryBuffer):
        ...
        if old_buffer.recent_buffer.buffer:
            n_keep = min(len(old_buffer.recent_buffer.buffer), new_buffer.recent_buffer.max_size)
