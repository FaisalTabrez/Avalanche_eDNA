...
class DNATokenizer:
    ...
    def _build_vocabulary(self) -> List[str]:
        ...
        if self.encoding_type in ["kmer", "both"]:
            ...
            kmers = [
                "".join(kmer)
                for kmer in itertools.product(nucleotides, repeat=self.kmer_size)
            ]
            vocab.extend(kmers)
    ...
    def encode_sequence(
        self, sequence: str, max_length: Optional[int] = None
    ) -> Dict[str, np.ndarray]:
        """
    ...
    def encode_sequences(
        ...
        return {
            ...
            "attention_mask": np.array(
                [enc["attention_mask"] for enc in encoded_sequences]
            ),
        }
