...
class TemperatureScaledConfidence:
    ...
    def is_uncertain(self, logits: torch.Tensor, threshold: float = 0.7) -> torch.Tensor:
        """
        ...
        for sample, cluster_id in zip(samples, cluster_ids):
            ...
            if len(self.buffer) < self.max_size:
                ...
                if random.random() < (priority / (total_priority + priority)):
                    ...
                    min_idx = min(range(len(self.metadata)), 
...
class ExemplarBuffer:
    ...
    def _select_diverse_exemplars(self, samples: np.ndarray, n_exemplars: int) -> np.ndarray:
        """
    ...
    def _update_centroids(self):
        ...
        clusters_to_keep = sorted(self.all_samples.keys())[-max(10, len(self.all_samples) // 10):]
        self.all_samples = {cid: self.all_samples[cid] for cid in clusters_to_keep}
    
    def sample(self, n_samples: int, exclude_cluster: Optional[int] = None) -> Tuple[np.ndarray, np.ndarray]:
        """
        ...
        Returns:
            ...
            cluster_ids: Corresponding cluster IDs [n_samples]
        """
        available_clusters = [cid for cid in self.exemplars.keys() 
                            if cid != exclude_cluster]
        ...
        for cluster_id in available_clusters:
            ...
            indices = np.random.choice(len(cluster_exemplars), n_from_cluster, replace=False)
    ...
    def get_stats(self) -> Dict:
        ...
        return {
            ...
            'last_centroid_update': self.clusters_added - (self.clusters_added % self.update_interval)
        }
...
class UncertaintyBuffer:
    ...
    def __init__(self, max_size: int = 50000, temperature: float = 2.0, 
                 threshold: float = 0.7):
        """
        Args:
    ...
    def add_batch(self, samples: np.ndarray, cluster_ids: np.ndarray, 
                  logits: torch.Tensor):
        """
...
class HybridMemoryBuffer:
    ...
    def mini_retrieval(self, 
                      n_exemplar: int = 4,
                      n_uncertain: int = 2,
                      n_recent: int = 2,
                      exclude_cluster: Optional[int] = None) -> Tuple[np.ndarray, np.ndarray]:
        """
    ...
    def sample_replay_batch(self, batch_size: int, 
                           exclude_cluster: Optional[int] = None) -> Tuple[np.ndarray, np.ndarray]:
        """
