...
class CheckpointManager:
    ...
    def __init__(
        self, checkpoint_dir: str, max_checkpoints: int = 5, keep_best: bool = True
    ):
        """
    ...
    def get_checkpoint_history(self) -> List[Dict[str, Any]]:
        ...
        return sorted(
            self.metadata.get("checkpoints", []), key=lambda x: x.get("epoch", 0)
        )

    def get_best_checkpoint_info(
        ...
        valid_checkpoints = [
            cp for cp in checkpoints if metric in cp.get("metrics", {})
        ]
