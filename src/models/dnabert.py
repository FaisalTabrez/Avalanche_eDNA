...
class DNABERTEmbedder:
    ...
    def encode(self, 
               ...
               batch_size: int = 8,
               pool_mode: str = 'mean',
               return_attention: bool = False) -> Union[np.ndarray, Dict[str, np.ndarray]]:
        """
        ...
        for i in range(0, len(prepared_seqs), batch_size):
            ...
            if return_attention:
                ...
                avg_attention = torch.stack(attention).mean(dim=0).mean(dim=1)  # (batch, seq_len, seq_len)
    ...
    def batch_similarity(self, 
                        sequences: List[str],
                        query: Optional[str] = None) -> np.ndarray:
        """
