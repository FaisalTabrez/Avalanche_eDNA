...
class DNABERTFineTuner:
    ...
    def prepare_scheduler(
        ...
        logger.info(
            f"Scheduler: {num_warmup_steps} warmup steps, {num_training_steps} total steps"
        )
        return scheduler

    def extract_embeddings(
        self, sequences: List[str], batch_size: int = 8, max_length: int = 512
    ) -> torch.Tensor:
        """
...
class ContrastiveLearningHead(nn.Module):
    ...
    def __init__(
        self, input_dim: int, projection_dim: int = 64, temperature: float = 0.07
    ):  # OPTIMIZED
        """
