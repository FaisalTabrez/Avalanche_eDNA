...
class ContinualLearner:
    ...
    def store_previous_model(self, model: nn.Module):
        ...
        if self.strategy in ["lwf", "combined"]:
            self.previous_model = type(model)(model.config).to(
    ...
    def get_combined_loss(
        ...
        if (
            self.strategy in ["lwf", "combined"]
            and current_outputs is not None
...
class ExperienceReplayBuffer:
    ...
    def sample(self, batch_size: int) -> Tuple[List[str], Optional[List[int]]]:
        ...
        sampled_labels = (
            [self.labels[i] for i in indices] if self.labels[0] is not None else None
        )
    ...
    def get_all_samples(self) -> Tuple[List[str], Optional[List[int]]]:
        ...
        return list(self.sequences), (
            list(self.labels) if self.labels[0] is not None else None
        )
