...
class EmbeddingTrainer:
    ...
    def __init__(
        ...
        if scaling_config is not None:
            if not SCALING_AVAILABLE:
                raise ImportError(
                    "Dynamic scaling requires DynamicHybridBuffer. Check installation."
                )
        ...
        if labels is not None:
            ...
            if isinstance(labels[0], str):
                ...
                labels_tensor = torch.tensor(
                    [label_to_id[label] for label in labels], dtype=torch.long
                )
            else:
    ...
    def train_autoencoder(
        ...
        for epoch in range(epochs):
            ...
            for batch in tqdm(train_loader, desc=f"Epoch {epoch+1}/{epochs}"):
                ...
                loss = criterion(
                    reconstructed,
                    F.one_hot(input_ids, num_classes=self.model.vocab_size).float(),
                )
    ...
    def train_contrastive(
        ...
        if not isinstance(self.model, DNAContrastiveModel):
            raise ValueError(
                "Model must be DNAContrastiveModel for contrastive training"
            )
    ...
    def train_with_dynamic_scaling(
        self,
        sequences: List[str],
        ...
        if self.dynamic_buffer is None:
            raise ValueError(
                "Dynamic scaling not enabled. Pass scaling_config to __init__"
            )
        ...
        if not isinstance(self.model, DNAContrastiveModel):
            raise ValueError(
                "Dynamic scaling currently only supports DNAContrastiveModel"
            )
        ...
        for cluster_idx, cluster_label in enumerate(unique_labels):
            logger.info(f"\n{'='*60}")
            logger.info(
                f"Training on Cluster {cluster_idx + 1}/{n_clusters} (Label: {cluster_label})"
            )
            logger.info(f"{'='*60}")
            ...
            encoded_train = self.tokenizer.encode_sequences(
                train_sequences, max_length=max_length
            )
            train_input_ids = torch.tensor(encoded_train["input_ids"], dtype=torch.long)
            train_attention_mask = torch.tensor(
                encoded_train["attention_mask"], dtype=torch.long
            )
            train_labels_tensor = torch.full(
                (len(train_sequences),), cluster_idx, dtype=torch.long
            )
            ...
            train_dataset = TensorDataset(
                combined_input_ids, combined_attention_mask, combined_labels_tensor
            )
            train_loader = DataLoader(
            ...
            if val_sequences:
                encoded_val = self.tokenizer.encode_sequences(
                    val_sequences, max_length=max_length
                )
                val_input_ids = torch.tensor(encoded_val["input_ids"], dtype=torch.long)
                val_attention_mask = torch.tensor(
                    encoded_val["attention_mask"], dtype=torch.long
                )
                val_labels_tensor = torch.full(
                    (len(val_sequences),), cluster_idx, dtype=torch.long
                )
            ...
            for epoch in range(epochs_per_task):
                ...
                logger.info(
                    f"Cluster {cluster_idx+1} Epoch {epoch+1}/{epochs_per_task} - "
            ...
            cluster_labels_array = np.full(
                len(cluster_embeddings), cluster_idx, dtype=np.int64
            )
            ...
            buffer_size = (
                buffer_stats["exemplar"].get("total_exemplars", 0)
                + buffer_stats["uncertainty"].get("size", 0)
                + buffer_stats["recent"].get("size", 0)
            )
            memory_usage = (
                buffer_size * cluster_embeddings.shape[1] * 4 / (1024**2)
            )  # Rough estimate
            ...
            history["exemplars_per_cluster"].append(
                self.scaling_config.exemplars_per_cluster
            )
            ...
            self.scaling_history["exemplars_per_cluster"].append(
                self.scaling_config.exemplars_per_cluster
            )
    ...
    def extract_embeddings(
        self, sequences: List[str], batch_size: int = 32, max_length: int = 512
    ) -> np.ndarray:
        """
        ...
        for i in range(0, len(sequences), batch_size):
            ...
            # Encode batch
            encoded = self.tokenizer.encode_sequences(
                batch_sequences, max_length=max_length
            )
            ...
            with torch.no_grad():
                ...
                from src.models.embeddings import DNAAutoencoder, DNAContrastiveModel
    ...
    def save_model(
        self, save_path: Union[str, Path], include_tokenizer: bool = True
    ) -> None:
        """
