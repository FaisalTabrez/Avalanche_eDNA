...
class DNATransformerEmbedder(nn.Module):
    ...
    def __init__(
        ...
        self.transformer = nn.TransformerEncoder(
            encoder_layer, num_layers=max(1, num_layers)
        )
...
class DNAContrastiveModel(nn.Module):
    ...
    def contrastive_loss(
        self, projections: torch.Tensor, labels: Optional[torch.Tensor] = None
    ) -> torch.Tensor:
        """
        ...
        if labels is not None:
            ...
            mask_positive = mask_positive.masked_fill(
                torch.eye(batch_size, dtype=torch.bool, device=device), 0
            )
            ...
            log_prob = (similarity_matrix - max_sim) - torch.log(
                exp_sim.sum(dim=1, keepdim=True) + eps
            )
            ...
            if batch_size % 2 != 0:
                raise ValueError(
                    "Batch size must be even for self-supervised contrastive learning"
                )

            # Create positive pair mask
            labels = torch.cat(
                [torch.arange(batch_size // 2), torch.arange(batch_size // 2)]
            ).to(device)
            labels = labels.contiguous().view(-1, 1)
...
class ModelFactory:
    ...
    @staticmethod
    def create_contrastive(
        backbone_model: nn.Module, config: Dict[str, Any]
    ) -> DNAContrastiveModel:
