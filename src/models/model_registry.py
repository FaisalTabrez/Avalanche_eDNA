...
class ModelRegistry:
    ...
    def __init__(self, registry_dir: str, backend: str = "json"):
        ...
        logger.info(
            f"ModelRegistry initialized with {backend} backend at {self.registry_dir}"
        )
    ...
    def list_models(
        self, status: Optional[str] = None, dataset: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        """
    ...
    def compare_models(self, version1: str, version2: str) -> Dict[str, Any]:
        ...
        comparison = {
            ...
            "dataset_differences": {
                "only_in_v1": list(
                    set(model1.get("datasets", [])) - set(model2.get("datasets", []))
                ),
                "only_in_v2": list(
                    set(model2.get("datasets", [])) - set(model1.get("datasets", []))
                ),
                "common": list(
                    set(model1.get("datasets", [])) & set(model2.get("datasets", []))
                ),
            },
            "created_at_diff_days": self._date_diff(
                model1.get("created_at"), model2.get("created_at")
            ),
        }
        ...
        for metric in all_metrics:
            ...
            if v1_val is not None and v2_val is not None:
                comparison["metric_differences"][metric] = {
                    ...
                    "percent_change": (
                        ((v2_val - v1_val) / v1_val * 100) if v1_val != 0 else 0
                    ),
                }
    ...
    def get_best_model(
        self, metric: str = "val_loss", minimize: bool = True
    ...
    def _list_models_sqlite(
        self, status: Optional[str], dataset: Optional[str]
    ) -> List[Dict[str, Any]]:
