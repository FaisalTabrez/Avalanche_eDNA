...
class TaxonomicMatcher:
    ...
    def calculate_taxonomic_similarity(
        self, taxonomy1: Dict[str, str], taxonomy2: Dict[str, str]
    ) -> float:
        """
...
class OrganismIdentifier:
    ...
    def batch_identify_organisms(
        self,
        sequence_groups: List[List[SeqRecord]],
        taxonomic_data: Optional[List[List[Dict[str, Any]]]] = None,
    ...
    def _extract_consensus_taxonomy(
        self,
        sequences: List[SeqRecord],
        taxonomic_assignments: Optional[List[Dict[str, Any]]] = None,
    ) -> Dict[str, str]:
    ...
    def _find_matching_organism(
        self, signature: str, taxonomy: Dict[str, str]
            ...
            with self.db_manager.get_connection() as conn:
                ...
                for organism_id in exact_matches:
                    ...
                    if organism:
                        ...
                        tax_similarity = (
                            self.taxonomic_matcher.calculate_taxonomic_similarity(
                ...
                if taxonomy.get("genus") and taxonomy.get("species"):
                    ...
                    for row in cursor.fetchall():
                        ...
                        if organism:
                            ...
                            sig_similarity = (
                                self.signature_generator.calculate_signature_similarity(
            ...
            with self.db_manager.get_connection() as conn:
                ...
                if count == 0:
                    return (
                        True,
                        0.9,
                    )  # Likely novel if no known organisms with same genus/species
                elif count < 3:
    ...
    def _calculate_confidence_score(
        self, sequences: List[SeqRecord], taxonomy: Dict[str, str]
    ) -> float:
        """
        ...
        taxonomic_levels_filled = sum(
            1
            for level in self.taxonomic_matcher.taxonomic_levels
            if taxonomy.get(level)
        )
        taxonomic_completeness = taxonomic_levels_filled / len(
            self.taxonomic_matcher.taxonomic_levels
        )
    ...
    def _generate_notes(
        ...
        if environmental_context:
            if environmental_context.get("collection_location"):
                notes.append(
                    f"Location: {environmental_context['collection_location']}"
                )
...
class OrganismMatcher:
    ...
    def match_organisms_across_reports(
            ...
            with self.db_manager.get_connection() as conn:
                ...
                for org_1 in unique_to_1:
                    for org_2 in unique_to_2:
                        ...
                        if (
                            similarity >= 0.8
                        ):  # High similarity threshold for cross-report matching
                ...
                return {
                    ...
                    "jaccard_similarity": (
                        len(exact_matches) / len(organisms_1.union(organisms_2))
                        if organisms_1.union(organisms_2)
                        else 0
                    ),
                }
    ...
    def _calculate_organism_similarity(
        self, organism_id_1: str, organism_id_2: str
    ) -> float:
        """
