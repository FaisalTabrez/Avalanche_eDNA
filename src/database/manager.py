...
from .models import (
    OrganismProfile, DatasetInfo, AnalysisReport, SimilarityMatrix,
    ReportComparison, AnalysisStatus, SequenceType, serialize_to_json
)
...
class DatabaseManager:
    ...
    def _insert_organism_profile(self, conn: sqlite3.Connection, profile: OrganismProfile):
        ...
        """, (
            profile.organism_id, profile.organism_name, profile.taxonomic_lineage,
            profile.kingdom, profile.phylum, profile.class_name, profile.order_name,
            profile.family, profile.genus, profile.species, profile.sequence_signature,
            profile.first_detected, profile.last_updated, profile.detection_count,
            profile.confidence_score, profile.is_novel_candidate, profile.novelty_score,
            json.dumps(profile.reference_databases) if profile.reference_databases else None,
            profile.notes
        ))
    
    def _update_organism_profile(self, conn: sqlite3.Connection, profile: OrganismProfile):
        ...
        conn.execute("""
            ...
            WHERE organism_id = ?
        """, (
            profile.organism_name, profile.taxonomic_lineage, profile.kingdom,
            profile.phylum, profile.class_name, profile.order_name, profile.family,
            profile.genus, profile.species, profile.sequence_signature,
            profile.last_updated, profile.confidence_score, profile.is_novel_candidate,
            profile.novelty_score,
            json.dumps(profile.reference_databases) if profile.reference_databases else None,
            profile.notes, profile.organism_id
        ))
    ...
    def store_dataset_info(self, dataset: DatasetInfo) -> bool:
        ...
        try:
            with self.get_connection() as conn:
                ...
                """, (
                    dataset.dataset_id, dataset.dataset_name, dataset.file_path,
                    dataset.file_format, dataset.file_size_mb, dataset.total_sequences,
                    dataset.sequence_type.value if dataset.sequence_type else None,
                    dataset.collection_date, dataset.collection_location,
                    dataset.depth_meters, dataset.temperature_celsius, dataset.ph_level,
                    dataset.salinity,
                    json.dumps(dataset.environmental_conditions) if dataset.environmental_conditions else None,
                    json.dumps(dataset.preprocessing_params) if dataset.preprocessing_params else None
                ))
    ...
    def store_analysis_report(self, report: AnalysisReport) -> bool:
        ...
        try:
            with self.get_connection() as conn:
                ...
                """, (
                    report.report_id, report.dataset_id, report.report_name,
                    report.analysis_type, report.status.value, report.processing_time_seconds,
                    report.min_length, report.max_length, report.mean_length,
                    report.median_length, report.std_length,
                    report.sequence_type_detected.value if report.sequence_type_detected else None,
                    json.dumps(report.composition_data) if report.composition_data else None,
                    report.shannon_diversity, report.simpson_diversity, report.evenness,
                    report.species_richness, report.n_clusters, report.silhouette_score,
                    report.cluster_coherence, report.sequences_with_taxonomy,
                    report.taxonomy_confidence_avg, report.novel_candidates_count,
                    report.novel_percentage, report.novelty_threshold,
                    report.full_report_path, report.results_json_path, report.visualizations_dir
                ))
    ...
    def list_analysis_reports(self, dataset_id: Optional[str] = None, 
                            limit: int = 100, offset: int = 0) -> List[AnalysisReport]:
        """
    ...
    def store_similarity_matrix(self, similarity: SimilarityMatrix) -> bool:
        ...
        try:
            with self.get_connection() as conn:
                ...
                """, (
                    similarity.comparison_id, similarity.report_id_1, similarity.report_id_2,
                    similarity.organism_overlap_count, similarity.organism_overlap_percentage,
                    similarity.jaccard_similarity, similarity.cosine_similarity,
                    similarity.kingdom_similarity, similarity.phylum_similarity,
                    similarity.genus_similarity, similarity.shannon_diversity_diff,
                    similarity.simpson_diversity_diff, similarity.evenness_diff,
                    similarity.cluster_structure_similarity, similarity.location_distance_km,
                    similarity.depth_difference_m, similarity.temporal_difference_days,
                    similarity.similarity_score, similarity.comparison_method
                ))
    ...
    def find_similar_reports(self, report_id: str, similarity_threshold: float = 0.7,
    ...
    def _row_to_organism_profile(self, row: tuple, description: Any) -> Optional[OrganismProfile]:
    ...
    def _row_to_analysis_report(self, row: tuple, description: Any) -> Optional[AnalysisReport]:
        ...
        try:
            ...
            if data.get('sequence_type_detected'):
                data['sequence_type_detected'] = SequenceType(data['sequence_type_detected'])
