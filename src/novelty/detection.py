    ...
    def _initialize_detector(self):
        ...
        if self.method == "isolation_forest":
            ...
            self.detector = EllipticEnvelope(
                contamination=self.kwargs.get("contamination", 0.1), random_state=42
            )
        else:
...
class DistanceBasedNoveltyDetector:
    ...
    def fit(self, embeddings: np.ndarray) -> "DistanceBasedNoveltyDetector":
        ...
        """
        logger.info(
            f"Fitting distance-based detector on {embeddings.shape[0]} reference samples"
        )
...
class EnsembleNoveltyDetector:
    ...
    def __init__(self, detectors: List[NoveltyDetector], voting: str = "soft"):
        ...
        logger.info(
            f"Ensemble detector initialized with {len(detectors)} base detectors"
        )
    ...
    def predict(self, embeddings: np.ndarray) -> np.ndarray:
        ...
        if self.voting == "hard":
            ...
            predictions = np.array(
                [detector.predict(embeddings) for detector in self.detectors]
            )
            ensemble_pred = np.array(
                [
                    stats.mode(predictions[:, i])[0][0]
            ...
            scores = np.array(
                [detector.decision_function(embeddings) for detector in self.detectors]
            )
...
class NoveltyAnalyzer:
    ...
    def analyze_novelty(
        ...
        if query_abundances is not None:
            ...
            ensemble_pred = np.where(
                abundance_mask, ensemble_pred, 1
            )  # Not novel if too rare
        ...
        logger.info(
            f"Novelty analysis complete. Found {len(novel_indices)} potential novel taxa"
        )

        return results

    def _analyze_cluster_coherence(
    ...
    def _analyze_novel_candidates(
        self,
        novel_embeddings: np.ndarray,
        reference_embeddings: np.ndarray,
        novel_sequences: List[str],
        novel_cluster_labels: Optional[np.ndarray] = None,
    ) -> Dict[str, Any]:
        ...
        if novel_cluster_labels is not None:
            ...
            for cluster_id in unique_clusters:
                ...
                cluster_sequences = [
                    novel_sequences[i] for i in np.where(cluster_mask)[0]
                ]
    ...
    def visualize_novelty_results(
        ...
        plt.scatter(
            reduced_embeddings[normal_mask, 0],
        ...
        plt.scatter(
            reduced_embeddings[novel_mask, 0],
...
def main():
    ...
    analyzer.visualize_novelty_results(
        query_embeddings, np.array(results["predictions"]["ensemble"])
    )
