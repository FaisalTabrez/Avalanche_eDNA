...
class EmbeddingClusterer:
    ...
    def __init__(self, method: str = "hdbscan", **kwargs):
        """
    ...
    def fit(self, embeddings: np.ndarray) -> np.ndarray:
        ...
        logger.info(
            f"Clustering {embeddings.shape[0]} sequences with {embeddings.shape[1]}-dim embeddings"
        )
        ...
        logger.info(
            f"Clustering complete: {len(np.unique(self.cluster_labels))} clusters found"
        )
    ...
    def _cluster_hierarchical(self, embeddings: np.ndarray) -> np.ndarray:
        ...
        linkage_matrix = linkage(
            distances, method=self.kwargs.get("linkage_method", "ward")
        )
        ...
        # Store additional information
        self.clusterer_info = {"linkage_matrix": linkage_matrix, "distances": distances}
    ...
    def plot_clusters(
        ...
        plt.ylabel("UMAP 2")
        plt.title(
            f"Sequence Clustering ({self.method.upper()})\n"
    ...
    def get_cluster_representatives(
        self, sequences: List[str], n_representatives: int = 5
        ...
        for label in unique_labels:
            ...
            representatives[int(label)] = [
                (cluster_sequences[i], float(distances[i])) for i in top_indices
            ]

        return representatives

    def save_results(
        self, sequences: List[str], save_dir: Path, include_embeddings: bool = False
    ) -> None:
        """
        ...
        with open(save_dir / "cluster_stats.txt", "w") as f:
            ...
            if self.cluster_stats["silhouette_score"] is not None:
                f.write(
                    f"Silhouette Score: {self.cluster_stats['silhouette_score']:.4f}\n"
                )

            if self.cluster_stats["calinski_harabasz_score"] is not None:
                f.write(
                    f"Calinski-Harabasz Score: {self.cluster_stats['calinski_harabasz_score']:.4f}\n"
                )
        ...
        with open(save_dir / "cluster_representatives.txt", "w") as f:
            for cluster_id, reps in representatives.items():
                ...
                for i, (seq, dist) in enumerate(reps, 1):
                    ...
                    f.write(
                        f"     Sequence: {seq[:100]}{'...' if len(seq) > 100 else ''}\n"
                    )
