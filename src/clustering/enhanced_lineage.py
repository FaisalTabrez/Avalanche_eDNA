...
class EnhancedLineage:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        result["lineage"] = {
            rank.rank_name: name for rank, name in self.lineage.items()
        }
        result["evidence"] = [
            {
...
class EnhancedTaxdumpResolver:
    ...
    def __init__(
        ...
        self.cache_db_path = cache_db_path or (
            self.taxdump_dir / "lineage_cache.db" if self.taxdump_dir else None
        )

        # Data structures
        self._name_to_taxid: Optional[Dict[str, int]] = None
        self._taxid_to_name: Optional[Dict[int, str]] = None
        self._common_names: Optional[Dict[int, List[str]]] = (
            None  # taxid -> [common names]
        )
    ...
    def _init_cache_db(self) -> None:
        ...
        try:
            if isinstance(self.cache_db_path, str) and self.cache_db_path not in (
                ":memory",
                ":memory:",
            ):
    ...
    def enhanced_lineage_by_taxid(self, taxid: Optional[int]) -> EnhancedLineage:
        ...
        assert (
            ...
            and self._taxid_to_name is not None
            and self._common_names is not None
            and self._synonyms is not None
        )
        ...
        lineage = EnhancedLineage(taxid=taxid, last_updated=datetime.now())
        ...
        while (
            current_taxid in self._nodes and current_taxid not in seen and steps < 100
        ):
    ...
    def _get_cached_lineage(self, taxid: int) -> Optional[EnhancedLineage]:
        ...
        try:
            cursor = self._cache_conn.cursor()
            cursor.execute(
                "SELECT lineage_json FROM lineage_cache WHERE taxid = ?", (taxid,)
            )
        ...
        if "evidence" in lineage_dict:
            for ev_data in lineage_dict["evidence"]:
                evidence = LineageEvidence(
                    ...
                    timestamp=(
                        datetime.fromisoformat(ev_data["timestamp"])
    ...
    def get_lineage_statistics(self) -> Dict[str, Any]:
        ...
        if not all(
            [self._name_to_taxid, self._nodes, self._common_names, self._synonyms]
        ):
            return {}
