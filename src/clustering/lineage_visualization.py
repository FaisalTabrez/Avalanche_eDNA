...
class TaxonomicTreeVisualizer:
    ...
    def create_hierarchical_tree(
        self, lineages: List[EnhancedLineage]
    ) -> Optional[go.Figure]:
            ...
            for rank in TaxonomicRank.basic_ranks():
                ...
                if rank_name:
                    ...
                    current[rank_name]["confidence"] = max(
                        current[rank_name]["confidence"], lineage.overall_confidence
                    )
        ...
        fig = go.Figure(
            go.Sunburst(
                ...
                marker=(
                    ...
                    if self.config.show_confidence
                    else dict(line=dict(width=2, color="white"))
                ),
            )
        )

        fig.update_layout(
            title=dict(text="Taxonomic Hierarchy", x=0.5, font=dict(size=16)),
        ...
        fig = go.Figure(
            data=[
                go.Pie(
                    ...
                    hole=0.4,
                    marker_colors=[
                        self.evidence_colors.get(EvidenceType(k), "#cccccc")
...
class LineageExporter:
            ...
            row = {
                ...
                **basic_lineage,
                ...
                "evidence_sources": "; ".join(
                    [ev.source.value for ev in lineage.evidence]
                ),
                "external_refs": "; ".join(
                    [
                        f"{ref.database}:{ref.identifier}"
                        for ref in lineage.external_refs
                    ]
                ),
                "last_updated": (
                    lineage.last_updated.isoformat() if lineage.last_updated else None
                ),
            }
            rows.append(row)
    ...
    def _export_xml(self, lineages: List[EnhancedLineage], output_path: Path) -> bool:
        ...
        for lineage in lineages:
            ...
            if lineage.scientific_name:
                xml_content.append(
                    f"    <scientific_name>{self._escape_xml(lineage.scientific_name)}</scientific_name>"
                )

            xml_content.append(
                f"    <confidence>{lineage.overall_confidence}</confidence>"
            )
            ...
            for rank, name in lineage.lineage.items():
                if name:
                    xml_content.append(
                        f"      <{rank.rank_name}>{self._escape_xml(name)}</{rank.rank_name}>"
                    )
            xml_content.append("    </taxonomy>")
            ...
            if lineage.evidence:
                ...
                for ev in lineage.evidence:
                    xml_content.append(
                        f'      <source type="{ev.source.value}" confidence="{ev.confidence}">'
                    )
                    if ev.method_details:
                        xml_content.append(
                            f"        <method>{self._escape_xml(ev.method_details)}</method>"
                        )
    ...
    def _export_json_ld(
        self, lineages: List[EnhancedLineage], output_path: Path
    ) -> bool:
        ...
        for lineage in lineages:
            ...
            taxon = {
                "@type": "taxon",
                "@id": (
                    f"urn:taxon:{lineage.taxid}"
                    if lineage.taxid
                    else f"urn:taxon:{hash(lineage.scientific_name or 'unknown')}"
                ),
    ...
    def _export_rdf(self, lineages: List[EnhancedLineage], output_path: Path) -> bool:
        ...
        for i, lineage in enumerate(lineages):
            ...
            if lineage.scientific_name:
                rdf_content.append(
                    f'    dwc:scientificName "{lineage.scientific_name}" ;'
                )
    ...
    def _export_phyloxml(
        ...
        for i, lineage in enumerate(lineages):
            ...
            xml_content.append(
                f'      <name>{self._escape_xml(lineage.scientific_name or f"taxon_{i}")}</name>'
            )
            ...
            xml_content.append(
                f'      <confidence type="bootstrap">{lineage.overall_confidence * 100:.1f}</confidence>'
            )
            xml_content.append(f"    </clade>")

        xml_content.extend(["  </phylogeny>", "</phyloxml>"])
...
class LineageReportGenerator:
    ...
    def generate_comprehensive_report(
        ...
        lineages: List[EnhancedLineage],
        output_dir: str,
        report_name: str = "lineage_report",
    ) -> str:
        ...
        self.exporter.export_lineages(
            lineages, output_dir / f"{report_name}.json", "json"
        )
        self.exporter.export_lineages(
            lineages, output_dir / f"{report_name}.csv", "csv"
        )
        self.exporter.export_lineages(
            lineages, output_dir / f"{report_name}.xml", "xml"
        )
    ...
    def _create_html_report(
        ...
        if PLOTLY_AVAILABLE:
            ...
            if tree_fig:
                ...
                html_parts.append(
                    f'<div id="tree-plot">{tree_fig.to_html(include_plotlyjs=False, div_id="tree-plot")}</div>'
                )

            if heatmap_fig:
                html_parts.append("<h3>Confidence Heatmap</h3>")
                html_parts.append(
                    f'<div id="heatmap-plot">{heatmap_fig.to_html(include_plotlyjs=False, div_id="heatmap-plot")}</div>'
                )

            if evidence_fig:
                html_parts.append("<h3>Evidence Distribution</h3>")
                html_parts.append(
                    f'<div id="evidence-plot">{evidence_fig.to_html(include_plotlyjs=False, div_id="evidence-plot")}</div>'
                )
        ...
        if stats and "rank_coverage" in stats:
            ...
            for rank, data in stats["rank_coverage"].items():
                html_parts.append(
                    f'<tr><td>{rank.title()}</td><td>{data["count"]}</td><td>{data["percentage"]:.1f}%</td></tr>'
                )
