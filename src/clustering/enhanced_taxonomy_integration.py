...
from .enhanced_lineage import (
    EnhancedLineage,
    EnhancedTaxdumpResolver,
    EvidenceType,
    LineageEvidence,
    TaxonomicRank,
    create_enhanced_resolver,
)
from .lineage_visualization import (
    LineageExporter,
    LineageReportGenerator,
    TaxonomicTreeVisualizer,
    create_visualizer_from_config,
)
from .multi_source_taxonomy import MultiSourceTaxonomyResolver, create_standard_resolver
from .taxdump_updater import TaxdumpUpdater, create_updater_from_config

# Import original taxonomy components
from .taxonomy import HybridTaxonomyAssigner, TaxdumpResolver
...
class EnhancedTaxonomySystem:
    ...
    def _initialize_components(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"Enhanced taxonomy system initialized (enhanced: {self.enable_enhanced})"
            )
    ...
    def _initialize_multi_source_resolver(self) -> None:
        ...
        try:
            ...
            if multi_source_config.get("enabled", False):
                self.multi_source_resolver = create_standard_resolver(
                    multi_source_config
                )
    ...
    ) -> Union[EnhancedLineage, Dict[str, Optional[str]]]:
        """
        ...
        if use_enhanced and self.enhanced_resolver:
            try:
                ...
                if identifier.isdigit():
                    ...
                    lineage = self.enhanced_resolver.enhanced_lineage_by_taxid(
                        int(identifier)
                    )
                else:
                    # Text - treat as scientific name
                    lineage = self.enhanced_resolver.enhanced_lineage_by_name(
                        identifier
                    )

                # Try multi-source resolution if primary failed
                if (
                    not lineage or not lineage.scientific_name
                ) and self.multi_source_resolver:
    ...
    def search_by_common_name(self, common_name: str) -> List[EnhancedLineage]:
        ...
        if self.enhanced_resolver:
            try:
                results.extend(
                    self.enhanced_resolver.search_by_common_name(common_name)
                )
            except Exception as e:
    ...
    def batch_lineage_lookup(
        self, identifiers: List[str], enhanced: bool = True
    ) -> List[Union[EnhancedLineage, Dict[str, Optional[str]]]]:
    ...
    def create_visualization_report(
        ...
        output_dir: str,
        report_name: str = "taxonomy_report",
    ) -> Optional[str]:
    ...
    def add_custom_taxonomy_override(
        self, scientific_name: str, custom_lineage: Dict[str, str]
    ) -> bool:
        ...
        try:
            ...
            enhanced_lineage = EnhancedLineage(
                scientific_name=scientific_name, last_updated=datetime.now()
            )
...
def create_enhanced_taxonomy_system(
    config_dict: Optional[Dict[str, Any]] = None,
) -> EnhancedTaxonomySystem:
...
class BackwardCompatibilityWrapper:
    ...
    def lineage_by_name(self, scientific_name: str) -> Dict[str, Optional[str]]:
        ...
        if isinstance(result, dict):
            ...
            return (
                result.get_basic_lineage()
                if hasattr(result, "get_basic_lineage")
                else {}
            )
    ...
    def available(self) -> bool:
        ...
        return (
            self.system.enhanced_resolver is not None
            or self.system.legacy_resolver is not None
        )
...
def initialize_enhanced_taxonomy() -> (
    Tuple[EnhancedTaxonomySystem, BackwardCompatibilityWrapper]
):
