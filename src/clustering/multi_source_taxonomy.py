...
class SILVATaxonomySource(TaxonomySource):
    ...
    def __init__(self, silva_file: Optional[str] = None):
        super().__init__(
            name="SILVA", description="SILVA ribosomal RNA gene database", priority=90
        )
    ...
    def _parse_silva_record(self, row: pd.Series) -> Optional[EnhancedLineage]:
        ...
        try:
            ...
            if silva_id:
                silva_ref = ExternalReference(database="SILVA", identifier=silva_id)
...
class PR2TaxonomySource(TaxonomySource):
    ...
    def __init__(self, pr2_file: Optional[str] = None):
        super().__init__(
            name="PR2", description="Protist Ribosomal Reference database", priority=85
        )
    ...
    def _parse_pr2_record(self, row: pd.Series) -> Optional[EnhancedLineage]:
        ...
        try:
            ...
            # Add external reference
            if pr2_id:
                pr2_ref = ExternalReference(database="PR2", identifier=pr2_id)
...
class CustomTaxonomySource(TaxonomySource):
    ...
    def _load_from_file(self) -> bool:
        ...
        try:
            if self.custom_file.suffix.lower() == ".json":
                ...
                logger.warning(
                    f"Unsupported custom taxonomy file format: {self.custom_file.suffix}"
                )
                return False
        except Exception as e:
    ...
    def _load_from_json(self) -> bool:
        ...
        self._loaded = True
        logger.info(
            f"Loaded {len(self._taxonomy_data)} custom taxonomy records from JSON"
        )
        return True
    ...
    def _load_from_csv(self) -> bool:
        ...
        self._loaded = True
        logger.info(
            f"Loaded {len(self._taxonomy_data)} custom taxonomy records from CSV"
        )
        return True
    ...
    def export_overrides(self, output_path: str) -> None:
        ...
        logger.info(
            f"Exported {len(overrides_data)} taxonomic overrides to {output_path}"
        )
...
class MultiSourceTaxonomyResolver:
    ...
    def add_source(self, source: TaxonomySource) -> None:
        ...
        logger.info(
            f"Added taxonomy source: {source.name} (priority: {source.priority})"
        )
    ...
    def add_mapping(self, mapping: TaxonomyMapping) -> None:
        ...
        logger.info(
            f"Added mapping: {mapping.source_name}:{mapping.source_id} -> {mapping.target_name}:{mapping.target_id}"
        )

    def resolve_lineage(
        self, identifier: str, prefer_source: Optional[str] = None
    ) -> Optional[EnhancedLineage]:
    ...
    def _resolve_conflicts(
        self, candidates: List[Tuple[TaxonomySource, EnhancedLineage]]
    ) -> EnhancedLineage:
        ...
        for source, lineage in candidates:
            ...
            for rank, name in lineage.lineage.items():
                if name and (
                    rank not in merged_lineage.lineage
                    or not merged_lineage.lineage[rank]
                ):
                    merged_lineage.lineage[rank] = name
                elif (
                    rank in merged_lineage.lineage
                    and merged_lineage.lineage[rank]
                    and name
                    and merged_lineage.lineage[rank] != name
                ):
    ...
    ) -> List[Tuple[str, EnhancedLineage]]:
...
def create_standard_resolver(
    config_dict: Optional[Dict[str, Any]] = None,
) -> MultiSourceTaxonomyResolver:
