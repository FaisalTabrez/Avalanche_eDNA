...
from src.database.models import ReportComparison, SimilarityMatrix
...
class SimilarityCalculator:
    ...
    def calculate_organism_overlap_similarity(
        self, organisms_1: Set[str], organisms_2: Set[str]
    ) -> Dict[str, float]:
        """
        ...
        overlap_coeff = (
            len(intersection) / min(len(organisms_1), len(organisms_2))
            if min(len(organisms_1), len(organisms_2)) > 0
            else 0.0
        )

        # Sorensen-Dice coefficient
        dice_coeff = (
            2 * len(intersection) / (len(organisms_1) + len(organisms_2))
            if (len(organisms_1) + len(organisms_2)) > 0
            else 0.0
        )
    ...
    def calculate_abundance_similarity(
        self, abundance_1: Dict[str, float], abundance_2: Dict[str, float]
    ) -> Dict[str, float]:
        """
        ...
        if len(vector_1) > 1:
            ...
            similarities["spearman_correlation"] = (
                spearman_corr if not np.isnan(spearman_corr) else 0.0
            )
            similarities["spearman_p_value"] = (
                spearman_p if not np.isnan(spearman_p) else 1.0
            )
        else:
        ...
        similarities["manhattan_similarity"] = (
            1 - (manhattan_dist / max_possible_dist) if max_possible_dist > 0 else 1.0
        )

        return similarities

    def calculate_taxonomic_composition_similarity(
        self,
        taxonomy_1: Dict[str, Dict[str, int]],
        taxonomy_2: Dict[str, Dict[str, int]],
    ) -> Dict[str, float]:
        """
        ...
        for level in taxonomic_levels:
            ...
            if level_comp_1 or level_comp_2:
                ...
                level_similarity = self.calculate_abundance_similarity(
                    level_comp_1, level_comp_2
                )
                similarities[f"{level}_similarity"] = level_similarity[
                    "cosine_similarity"
                ]
            else:
        ...
        similarities["overall_taxonomic_similarity"] = (
            weighted_sum / total_weight if total_weight > 0 else 0.0
        )

        return similarities

    def calculate_diversity_similarity(
        self, diversity_1: Dict[str, float], diversity_2: Dict[str, float]
    ) -> Dict[str, float]:
        """
        ...
        for metric in diversity_metrics:
            ...
            if val_1 == 0.0 and val_2 == 0.0:
                ...
                similarities[f"{metric}_similarity"] = (
                    1 - (difference / max_val) if max_val > 0 else 1.0
                )

        # Calculate overall diversity similarity
        metric_similarities = [
            similarities[f"{metric}_similarity"] for metric in diversity_metrics
        ]
        similarities["overall_diversity_similarity"] = np.mean(metric_similarities)

        return similarities

    def calculate_environmental_similarity(
        self, env_1: Dict[str, Any], env_2: Dict[str, Any]
    ) -> Dict[str, float]:
        """
        ...
        if (
            env_1.get("latitude")
            and env_1.get("longitude")
            and env_2.get("latitude")
            and env_2.get("longitude")
        ):
            ...
            similarities["geographic_similarity"] = np.exp(
                -distance_km / 100
            )  # 100km scale
        else:
...
class CrossAnalysisEngine:
    ...
    def compare_reports(
        ...
        try:
            ...
            comparison_id = SimilarityMatrix.generate_comparison_id(
            ...
            try:
                ...
                jaccard_sim = (
                    shared_organisms / total_organisms if total_organisms > 0 else 0.0
                )
            ...
            logger.info(
                f"Report comparison completed. Overall similarity: {overall_similarity:.3f}"
            )
        ...
        try:
            with self.db_manager.get_connection() as conn:
                ...
                if total_sequences > 0:
                    organisms = {
                        org_id: count / total_sequences
                        for org_id, count in organisms.items()
                    }
    ...
    def _get_report_environmental_context(self, report_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(
                f"Failed to get environmental context for report {report_id}: {str(e)}"
            )
            return {}
    ...
    def _calculate_overall_similarity(
        ...
        environmental_sim: Dict[str, float],
    ) -> float:
        ...
        scores = {
            ...
            "taxonomic_similarity": taxonomic_sim.get(
                "overall_taxonomic_similarity", 0.0
            ),
            "diversity_similarity": diversity_sim.get(
                "overall_diversity_similarity", 0.0
            ),
