    ...
    def _download_from_ena(self, accession: str, output_dir: Path) -> Optional[Path]:
        ...
        try:
            ...
            with urllib.request.urlopen(ena_api_url, timeout=30) as response:
                ...
                urllib.request.urlretrieve(http_url, output_file, reporthook=download_progress)
                
                if output_file.exists():
                    logger.info(f"Successfully downloaded {accession} from ENA to {output_file}")
                    return output_file
    ...
    def download_sra_dataset(self, accession: str, output_dir: Path, 
        ...
        if self.sra_toolkit_available:
            try:
                ...
                logger.warning(f"SRA Toolkit method failed: {e}, falling back to ENA...")

        # Fallback to ENA download (works without SRA Toolkit)
        logger.info("Using ENA direct download method...")
        if progress_callback:
            progress_callback("Downloading from ENA mirror (no SRA Toolkit required)...")
