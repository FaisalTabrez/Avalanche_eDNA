...
class WindowsBLASTRunner:
    ...
    def create_blast_database(
        self, fasta_file: str, database_name: str, database_type: str = "nucl"
    ) -> bool:
        """
        ...
        try:
            ...
            logger.debug(
                f"Output directory exists: {Path(database_name).parent.exists()}"
            )
            ...
            parent_dir = (
                database_path.parent if database_path.parent.exists() else Path(".")
            )
    ...
    def _parse_blast_xml(
        self, xml_file: str, sequence_ids: List[str]
    ) -> List[Dict[str, Any]]:
        """
...
def create_database(
    fasta_file: str, database_name: str, database_type: str = "nucl"
) -> bool:
...
def search_sequences(
    sequences: List[str], database_path: str, sequence_ids: Optional[List[str]] = None
) -> List[Dict[str, Any]]:
