...
class ReportCatalogueManager:
    ...
    def __init__(
        ...
        if storage_root is None:
            storage_root = (
                Path(__file__).parent.parent.parent / "data" / "report_storage"
            )
    ...
    def store_analysis_report(
        ...
        dataset_info = self._create_dataset_info(
            dataset_file_path, environmental_context
        )
    ...
    def retrieve_analysis_report(self, report_id: str) -> Optional[Dict[str, Any]]:
        ...
        results_path = (
            Path(report.results_json_path) if report.results_json_path else None
        )
        detailed_results = {}
    ...
    def export_report(
        self, report_id: str, export_format: str = "json"
    ) -> Optional[str]:
    ...
    def _generate_report_id(
        self, dataset_file_path: str, analysis_results: Dict[str, Any]
    ) -> str:
        """Generate unique report ID."""
    ...
    def _create_dataset_info(
        self, dataset_file_path: str, environmental_context: Optional[Dict[str, Any]]
    ) -> DatasetInfo:
        ...
        if environmental_context:
            ...
            dataset_info.collection_location = environmental_context.get(
                "collection_location"
            )
            dataset_info.depth_meters = environmental_context.get("depth_meters")
            dataset_info.temperature_celsius = environmental_context.get(
                "temperature_celsius"
            )
    ...
    def _create_storage_path(self, report_id: str) -> Path:
        ...
        now = datetime.now()
        year_month_path = (
            self.storage_root / "reports" / str(now.year) / f"{now.month:02d}"
        )
    ...
    def _store_dataset_file(self, dataset_file_path: str, storage_path: Path) -> Path:
        ...
        if (
            source_path.stat().st_size > (self.compression_threshold_mb * 1024 * 1024)
            and self.compress_large_files
        ):
    ...
    def _generate_comprehensive_report(
        ...
        lines.append(
            f"- **Processing Time:** {analysis_report.processing_time_seconds:.2f} seconds"
        )
        if analysis_report.shannon_diversity:
            lines.append(
                f"- **Shannon Diversity:** {analysis_report.shannon_diversity:.4f}"
            )
        lines.append("")
