...
class eDNABiodiversityPipeline:
    ...
    def run_complete_pipeline(
        ...
        try:
            ...
            if run_preprocessing:
                ...
                self.results["pipeline_config"]["steps_completed"].append(
                    "preprocessing"
                )
            else:
            ...
            if run_embedding:
                ...
                embeddings = self._run_embedding_step(
                    sequences, output_dir, custom_model_path
                )
            ...
            if run_clustering:
                ...
                cluster_results = self._run_clustering_step(
                    embeddings, sequences, output_dir
                )
            ...
            if run_taxonomy:
                ...
                taxonomy_results = self._run_taxonomy_step(
                    sequences, embeddings, output_dir
                )
    ...
    def _run_preprocessing_step(self, input_data: str, output_dir: Path) -> List[str]:
        ...
        if input_path.is_dir():
            ...
            results = [
                self.preprocessing_pipeline.process_file(input_path, output_prefix)
            ]
    ...
    def _is_sra_data(self, input_path: Path) -> bool:
        ...
        if input_path.is_dir():
            ...
            return (
                input_path.suffix.lower() in [".sra", ".fastq", ".fq", ".gz"]
                or name_lower.endswith(".fastq.gz")
                or name_lower.endswith(".fq.gz")
            )
    ...
    def _process_sra_data(self, input_path: Path, output_dir: Path) -> List[str]:
        ...
        try:
            ...
            raise ImportError(
                "SRA processor not available. Please ensure src/preprocessing/sra_processor.py exists."
            ) from e
        ...
        if input_path.is_dir():
            ...
            if fastq_files:
                ...
                if fastq_files:
                    results = sra_processor.integrate_with_pipeline(
    ...
    def _run_embedding_step(
        ...
        if custom_model_path:
            ...
            return self._extract_custom_embeddings(
                sequences, custom_model_path, output_dir
            )
        ...
        try:
            ...
            logger.warning(f"Failed to load {model_id}: {e}. Trying CPU-native DNABERT-1...")
                    ...
                    for ch in id_chunks:
                        inputs = {
                            "input_ids": torch.tensor(
                                [ch], dtype=torch.long, device=device
                            )
                        }
        ...
        for i in range(0, len(sequences), batch_size):
            ...
            logger.info(
                f"Embedded {min(i+batch_size, len(sequences))}/{len(sequences)} sequences"
            )
        ...
        try:
            if isinstance(post_cfg, dict) and pca_cfg.get("enabled", True):
                ...
                logger.info(
                    f"Applying PCA to {n_components} components on embeddings of shape {embeddings.shape}"
                )
                pca = PCA(
        ...
        try:
            l2_norm = (
                ...
                else bool(post_cfg.get("l2_normalize", True))
            )
            if l2_norm:
            ...
            logger.warning(
                f"L2 normalization failed ({e}). Continuing without normalization."
            )
        ...
        logger.info(
            f"Embeddings generated via DNABERT-2 ({model_id}): {embeddings.shape}"
        )
        return embeddings

    def _run_clustering_step(
        self, embeddings: np.ndarray, sequences: List[str], output_dir: Path
    ) -> Dict[str, Any]:
        """Run clustering step"""
        ...
        self.clusterer.save_results(
            sequences, clustering_output_dir, include_embeddings=True
        )
    ...
    def _run_taxonomy_step(
        self, sequences: List[str], embeddings: np.ndarray, output_dir: Path
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            emb_path = Path(
                knn_cfg.get("embeddings_path", ref_dir / "reference_embeddings.npy")
            )
            labels_path = Path(
                knn_cfg.get("labels_path", ref_dir / "reference_labels.csv")
            )
            if emb_path.exists() and labels_path.exists():
                ...
                index = TaxonomyIndex(
                    ...
                    labels_df,
                    normalize=True,
                    index_type=knn_cfg.get("index", "flat_ip"),
                )
                knn_assigner = KNNLCATaxonomyAssigner(
                    ...
                    min_agreement=knn_cfg.get(
                        "min_agreement", {"species": 0.8, "genus": 0.7, "family": 0.6}
                    ),
                )
            else:
                logger.warning(
                    f"Reference files not found for KNN taxonomy: {emb_path} or {labels_path}"
                )
        except Exception as e:
        ...
        if bool(blast_cfg.get("enable", True)):
            try:
                blast_assigner = BlastTaxonomyAssigner(
                    ...
                    max_targets=int(
                        taxonomy_cfg.get("blast", {}).get("max_targets", 5)
                    ),
                    identity_threshold=float(
                        blast_cfg.get(
                            "min_identity_species",
                            taxonomy_cfg.get("blast", {}).get(
        ...
        try:
            cluster_labels = (
                np.array(self.results.get("clustering", {}).get("cluster_labels"))
        ...
        if knn_assigner is None and blast_assigner is not None:
            try:
                logger.info(
                    "Running pure BLAST taxonomy assignment (no KNN index available)..."
                )
                assignments = blast_assigner.assign_taxonomy(
                    sequences, sequence_ids=sequence_ids
                )
                    ...
                    a.update(
                        {
                            "assigned_rank": (
                                "species"
                                if a.get("taxonomy")
                                and a.get("identity", 0.0)
                                >= blast_assigner.identity_threshold
                                else None
                            ),
                            "assigned_label": a.get("taxonomy"),
                            "confidence": (
                                min(0.99, a.get("identity", 0.0) / 100.0)
            ...
            hybrid = HybridTaxonomyAssigner(
                ...
                cluster_consensus_threshold=float(
                    consensus_cfg.get("min_cluster_agreement", 0.7)
                ),
            )
        ...
        for a in assignments:
            label = (
                a.get("assigned_label", "Unknown")
                if a.get("assigned_label")
                else "Unknown"
            )
        ...
        try:
            ...
            blast_thr = float(
                taxonomy_cfg.get("blast", {}).get("identity_threshold", 97.0)
            )

            def decide(row):
                knn_sp = (
                    row.get("knn_label") if row.get("knn_rank") == "species" else None
                )
                blast_sp = row.get("blast_label") or row.get("taxonomy")
                blast_id = row.get("blast_identity")
                knn_conf = row.get("knn_confidence")
                conflict = (
                    isinstance(knn_sp, str)
                    and isinstance(blast_sp, str)
                    and knn_sp
                    and blast_sp
                    and (knn_sp.strip() != blast_sp.strip())
                )
            ...
            predictions_df = pd.concat(
                [predictions_df.reset_index(drop=True), tb.reset_index(drop=True)],
                axis=1,
            )
        except Exception as e:
        ...
        logger.info(
            f"Taxonomy assignment complete: {len(taxonomy_counts)} unique labels identified"
        )
        return results

    def _run_novelty_step(
        self,
        embeddings: np.ndarray,
        sequences: List[str],
        cluster_results: Optional[Dict],
        output_dir: Path,
    ) -> Dict[str, Any]:
        ...
        self.novelty_analyzer.visualize_novelty_results(
            embeddings, ensemble_predictions, viz_file
        )
        ...
        logger.info(
            f"Novelty detection complete: {results['novel_candidates']} novel candidates found"
        )
        return results
    ...
    def _calculate_summary_statistics(self) -> None:
        ...
        summary = {
            "total_sequences_processed": self.results.get("preprocessing", {}).get(
                "total_sequences", 0
            ),
            "total_clusters": self.results.get("clustering", {}).get("n_clusters", 0),
            "total_taxa_identified": len(
                self.results.get("taxonomy", {}).get("taxonomy_counts", {})
            ),
            "novel_taxa_candidates": self.results.get("novelty", {}).get(
                "novel_candidates", 0
            ),
            "novelty_percentage": self.results.get("novelty", {}).get(
                "novel_percentage", 0
            ),
        }
    ...
    def _run_training_step(
        self, sequences: List[str], output_dir: Path
    ) -> Dict[str, Any]:
        """Train a custom embedding model on the dataset"""
        try:
            from src.models.embeddings import (
                DNAContrastiveModel,
                DNATransformerEmbedder,
                ModelFactory,
            )
        ...
        num_layers = (
            self.config.get("embedding", {}).get("transformer", {}).get("num_layers", 6)
        )
        num_heads = (
            self.config.get("embedding", {}).get("transformer", {}).get("num_heads", 8)
        )
        ...
        self.trainer = EmbeddingTrainer(
            self.embedding_model, self.tokenizer, device=device
        )
    ...
    def _extract_custom_embeddings(
        self, sequences: List[str], model_path: str, output_dir: Path
    ) -> np.ndarray:
        ...
        backbone = DNATransformerEmbedder(
            vocab_size=self.tokenizer.vocab_size, d_model=256, num_layers=6
        )
        self.embedding_model = DNAContrastiveModel(
            backbone_model=backbone, projection_dim=64  # OPTIMIZED
        )

        # Create trainer and load model
        self.trainer = EmbeddingTrainer(
            self.embedding_model, self.tokenizer, device="auto"
        )
    ...
    def _initialize_continual_learning(self, output_dir: Path):
        ...
        self.checkpoint_manager = CheckpointManager(checkpoint_dir=str(checkpoint_dir))

        self.model_registry = ModelRegistry(
            registry_dir=str(registry_dir), backend="json"  # Use JSON for simplicity
        )
    ...
    def _resume_from_checkpoint(self, checkpoint_path: str):
        ...
        if not self.checkpoint_manager:
            raise ValueError(
                "CheckpointManager not initialized. Call _initialize_continual_learning first."
            )
        ...
        if self.embedding_model and "model_state_dict" in checkpoint_data:
            ...
            logger.info(
                f"Resumed from checkpoint: epoch {checkpoint_data.get('epoch', 0)}"
            )
    ...
    def _save_checkpoint_if_enabled(
        ...
        if checkpoint_every > 0 and epoch % checkpoint_every == 0:
            ...
            checkpoint_data = {"epoch": epoch, "metrics": metrics or {}}
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="eDNA Biodiversity Assessment Pipeline"
    )

    parser.add_argument(
        "--input", type=str, required=True, help="Input data path (file or directory)"
    )
    ...
    parser.add_argument(
        "--skip-embedding", action="store_true", help="Skip embedding generation"
    )
    parser.add_argument(
        "--skip-clustering", action="store_true", help="Skip clustering step"
    )
    parser.add_argument(
        "--skip-taxonomy", action="store_true", help="Skip taxonomy assignment"
    )
    parser.add_argument(
        "--skip-novelty", action="store_true", help="Skip novelty detection"
    )
    parser.add_argument(
        "--skip-visualization",
        action="store_true",
