...
def demonstrate_scaling_configs():
    ...
    print("\n" + "─" * 80)
    print(
        f"{'Clusters':<10} {'Exemplars':<12} {'Uncertainty':<15} {'Recent':<12} {'Temp':<8} {'Strategy':<15}"
    )
    print("─" * 80)
    ...
    for n_clusters in scales:
        ...
        print(
            f"{n_clusters:<10} {config.exemplars_per_cluster:<12} "
...
def demonstrate_memory_scaling():
    ...
    for n_clusters in scales:
        ...
        print(f"  {'─'*40}")
        print(
            f"  Total:              {memory['total_mb']:>8.1f} MB / {memory['budget_mb']:.0f} MB"
        )
...
def demonstrate_dynamic_adaptation():
    ...
    for cluster_id in range(100):
        ...
        if cluster_id + 1 in adaptation_points:
            ...
            print(f"    Exemplars/cluster: {stats['config']['exemplars_per_cluster']}")
            print(
                f"    Uncertainty buffer: {stats['config']['uncertainty_buffer_size']:,}"
            )
    ...
    print(
        f"  Total exemplars: {stats['buffer_stats']['exemplar']['total_exemplars']:,}"
    )
...
def demonstrate_configuration_comparison():
    ...
    print("\n" + "─" * 80)
    print(
        f"{'Target Acc':<12} {'Exemplars':<12} {'Uncertainty':<15} {'Total Memory':<15}"
    )
    print("─" * 80)
    ...
    for target in targets:
        ...
        print(
            f"{target:<12.0%} {config.exemplars_per_cluster:<12} "
...
def main():
    ...
    print(
        "  • System adapts at: 10, 20, 50, 100, 200, 500, 1000, 2000, 5000, 10000 clusters"
    )
