...
def demonstrate_mini_retrieval():
    ...
    buffer = HybridMemoryBuffer(
        exemplars_per_cluster=20, uncertainty_size=100, recent_size=100, temperature=2.0
    )
    ...
    for cluster_id in range(10):
        ...
        if cluster_id >= 3:  # Start mini-retrieval after 4 clusters
            ...
            replay_samples, replay_labels = buffer.mini_retrieval(
                n_exemplar=4, n_uncertain=2, n_recent=2, exclude_cluster=cluster_id
            )
            ...
            if cluster_id == 5:
                ...
                print(f"\n  Breakdown:")
                print(
                    f"    Exemplar samples (~4): likely from clusters {list(source_counts.keys())[:2]}"
                )
...
def demonstrate_centroid_updates():
    ...
    buffer = ExemplarBuffer(
        exemplars_per_cluster=10, update_interval=5  # Update every 5 clusters
    )
...
def demonstrate_full_training_workflow():
    ...
    for cluster_id in range(n_clusters):
        ...
        for batch_idx in range(n_batches):
            ...
            batch_indices = np.random.choice(
                len(cluster_samples), batch_size // 2, replace=False
            )
    ...
    print(
        f"  • Exemplar: {stats['exemplar']['total_exemplars']:,} "
        f"({stats['exemplar']['n_clusters']} clusters × 20)"
    )
    print(
        f"  • Uncertainty: {stats['uncertainty']['size']:,} "
