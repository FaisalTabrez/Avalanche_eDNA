...
def load_labels(
    labels_path: Optional[str], sequences: List[str]
) -> Optional[List[str]]:
    """
    ...
    if len(labels) != len(sequences):
        raise ValueError(
            f"Number of labels ({len(labels)}) does not match number of sequences ({len(sequences)})"
        )
...
def train_model(args):
    ...
    tokenizer = DNATokenizer(
        encoding_type="kmer", kmer_size=args.kmer_size, stride=args.kmer_stride
    )
    ...
    if args.enable_scaling:
        if not labels:
            logger.error(
                "Dynamic scaling requires labels. Please provide --labels argument."
            )
            sys.exit(1)
        ...
        try:
            ...
            logger.info(
                f"Initializing dynamic scaling for {n_clusters} clusters, {dataset_size} samples"
            )
            ...
            logger.info(
                f"Scaling config: {scaling_config.exemplars_per_cluster} exemplars/cluster, "
                f"{scaling_config.uncertainty_buffer_size} uncertainty buffer"
            )

        except ImportError:
            logger.error(
                "Dynamic scaling not available. Install required dependencies."
            )
            sys.exit(1)
    ...
    if args.resume:
        ...
        if args.model_type:
            model, model_name = create_model(
                args.model_type, tokenizer.vocab_size, config_dict
            )
        else:
            # Try to infer from checkpoint metadata
            logger.warning("Model type not specified, defaulting to contrastive")
            model, model_name = create_model(
                "contrastive", tokenizer.vocab_size, config_dict
            )

        trainer = EmbeddingTrainer(
            model, tokenizer, device=args.device, scaling_config=scaling_config
        )
        ...
        metadata.update(
            {
                ...
                "target_accuracy": args.target_accuracy,
                "final_buffer_size": (
                    history["buffer_size"][-1] if "buffer_size" in history else None
                ),
                "final_memory_mb": (
                    history["memory_mb"][-1] if "memory_mb" in history else None
                ),
                "exemplars_per_cluster": (
                    history["exemplars_per_cluster"][-1]
                    if "exemplars_per_cluster" in history
                    else None
                ),
            }
        )
...
def main():
    ...
    # Input/Output
    parser.add_argument(
        "--input", "-i", required=True, help="Input FASTA file with DNA sequences"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--config", "-c", default=None, help="Custom configuration YAML file"
    )

    # Model configuration
    parser.add_argument(
        "--model-type",
        choices=["transformer", "autoencoder", "contrastive"],
        default="contrastive",
    ...
    # Resume training
    parser.add_argument(
        "--resume", default=None, help="Resume training from checkpoint directory"
    )
