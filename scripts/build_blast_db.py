...
def build_db_with_utils(fasta: Path, db_out: Path, taxid_map: Path | None) -> None:
    ...
    success = blast_runner.create_blast_database(
        fasta_file=str(fasta), database_name=str(db_out), database_type="nucl"
    )

    if not success:
        raise RuntimeError(
            "Failed to create BLAST database using Windows BLAST utilities"
        )
...
def build_db_basic(fasta: Path, db_out: Path, taxid_map: Path | None) -> None:
    ...
    if not makeblastdb_cmd:
        raise RuntimeError(
            "makeblastdb not found. Please install BLAST+ or check PATH."
        )
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Build BLAST DB from FASTA with optional taxid map"
    )
    ap.add_argument("--fasta", required=True, type=str, help="Input FASTA file path")
    ap.add_argument(
        "--db-out", required=True, type=str, help="Output DB prefix path (no extension)"
    )
    ap.add_argument(
        "--taxid-map", type=str, help="TSV with columns: sequence_id<tab>taxid"
    )
    ap.add_argument(
        "--force-basic", action="store_true", help="Force use of basic implementation"
    )
    args = ap.parse_args()
