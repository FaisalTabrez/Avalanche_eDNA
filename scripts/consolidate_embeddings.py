...
def build_consolidated_index(
    ...
    if incremental:
        ...
        existing_embeddings, all_metadata, processed_runs = load_existing_reference(
            output_dir
        )
        if existing_embeddings is not None:
            print(
                f"   Existing index: {len(existing_embeddings)} sequences from {len(processed_runs)} runs"
            )
        else:
    ...
    print(f"\nFiles created:")
    print(
        f"  - consolidated_embeddings.npy  ({all_embeddings.nbytes / 1024 / 1024:.1f} MB)"
    )
...
def main():
    ...
    parser.add_argument(
        "--incremental", action="store_true", help="Only add new runs to existing index"
    )
    parser.add_argument(
        "--compress",
        action="store_true",
