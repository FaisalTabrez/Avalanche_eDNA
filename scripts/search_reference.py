...
def get_query_embedding(
    ...
    if run_path and seq_idx is not None:
        ...
        if seq_idx >= len(emb):
            raise IndexError(
                f"Sequence index {seq_idx} out of range (max: {len(emb)-1})"
            )
...
def main():
    ...
    query_group.add_argument("--query", type=str, help="Query sequence string")
    query_group.add_argument(
        "--query-file", type=Path, help="Query sequence FASTA file"
    )
    query_group.add_argument(
        "--run", type=str, help="Run path (e.g., My_Dataset/2024-11-22_10-30-45)"
    )

    parser.add_argument(
        "--seq-idx", type=int, help="Sequence index in run (required with --run)"
    )
    parser.add_argument(
        "--top-k",
        type=int,
