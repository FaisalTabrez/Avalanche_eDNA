...
def compress_embedding(
    npy_path: Path, verify: bool = True, delete_original: bool = False
) -> bool:
    """
    ...
    try:
        ...
        print(
            f"   ✓ {npy_path.name}: {original_size/1024/1024:.1f}MB → {compressed_size/1024/1024:.1f}MB "
    ...
    for npy_file in npy_files:
        ...
        if args.execute:
            success = compress_embedding(
                npy_file, verify=True, delete_original=args.delete_original
            )
            ...
            if success:
                ...
                total_original_size += (
                    npy_file.stat().st_size if npy_file.exists() else 0
                )
            else:
                skipped_count += 1
        else:
