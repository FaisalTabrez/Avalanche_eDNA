...
def plot_comparison():
    ...
    strategies = ["Passive\nReplay", "Active\nReplay"]
    overall_accs = [
        passive_results["overall_accuracy"],
        active_results["overall_accuracy"],
    ]
    ...
    axes[0, 0].axhline(
        y=20, color="gray", linestyle="--", alpha=0.5, label="Random (20%)"
    )
    ...
    cluster_data = np.array(
        [passive_results["cluster_accuracies"], active_results["cluster_accuracies"]]
    )

    im = axes[0, 1].imshow(
        cluster_data.T, cmap="RdYlGn", aspect="auto", vmin=0, vmax=100
    )
    ...
    axes[0, 1].set_title(
        "Per-Cluster Accuracy Retention", fontsize=14, fontweight="bold"
    )
    ...
    axes[1, 0].set_title(
        "Strategy Configuration", fontsize=14, fontweight="bold", pad=20
    )
    ...
    axes[1, 1].set_title(
        "Cluster-wise Retention Pattern", fontsize=14, fontweight="bold"
    )
    ...
    print("-" * 70)
    print(
        f"{'Passive Replay':<20} {passive_results['buffer_size']:<10} {passive_results['overall_accuracy']:>6.1f}%        {'SEVERE (only last cluster)'}"
    )
    print(
        f"{'Active Replay':<20} {active_results['buffer_size']:<10} {active_results['overall_accuracy']:>6.1f}%        {'TBD'}"
    )
    print("=" * 70)

    if active_results["overall_accuracy"] > 0:
        improvement = (
            active_results["overall_accuracy"] - passive_results["overall_accuracy"]
        )
        print(f"\n🎯 Improvement: {improvement:+.1f} percentage points")

        # Count how many clusters retained
        passive_retained = sum(
            1 for acc in passive_results["cluster_accuracies"] if acc > 50
        )
        active_retained = sum(
            1 for acc in active_results["cluster_accuracies"] if acc > 50
        )
