...
class CompletePipeline:
    ...
    def __init__(
        ...
        for subdir in ['embeddings', 'clustering', 'models', 'checkpoints', 'visualizations']:
        ...
        self.dnabert_model = AutoModel.from_pretrained(model_path, trust_remote_code=True)
        self.dnabert_model.eval()
        print(f"✓ DNABERT-2 loaded ({sum(p.numel() for p in self.dnabert_model.parameters()):,} parameters)")
    ...
    def step2_generate_embeddings(self, batch_size: int = 32):
        ...
        for i in range(0, total, batch_size):
            ...
            if (i + batch_size) % 320 == 0 or i + batch_size >= total:
                ...
                print(f"  Progress: {progress:>5}/{total} ({100*progress/total:>5.1f}%) - {rate:.0f} seqs/sec")
    ...
    def step3_cluster_sequences(self):
        ...
        for cluster_id in range(self.n_clusters):
            cluster_seqs = [self.sequences[i] for i, l in enumerate(self.cluster_labels) if l == cluster_id]
    ...
    def step4_train_continual_learning(self, epochs_per_cluster: int = 10):
        ...
        for cluster_id in range(self.n_clusters):
            ...
            for epoch in range(epochs_per_cluster):
                ...
                for i in range(0, len(cluster_indices), batch_size):
                    ...
                    if cluster_id > 0 and continual_learner.strategy in ['replay', 'combined']:
                        if len(continual_learner.replay_buffer.sequences) > 0:
                            # Sample from replay buffer (50% of batch)
                            replay_size = min(batch_size // 2, len(continual_learner.replay_buffer.sequences))
                            replay_samples = continual_learner.replay_buffer.sample(replay_size)
                            
                            if replay_samples and replay_samples[0]:  # Check if we got valid samples
                    ...
                    if cluster_id > 0 and continual_learner.strategy in ['ewc', 'combined']:
                    ...
                    if continual_learner.strategy in ['replay', 'combined']:
                        ...
                        continual_learner.replay_buffer.add_samples(batch_seqs_str, batch_labels)
                ...
                if (epoch + 1) % 2 == 0:
                    replay_info = f", Replay batches: {replay_batches_used}" if cluster_id > 0 else ""
                    print(f"  Epoch {epoch+1}/{epochs_per_cluster}: Loss={avg_loss:.4f}, Acc={accuracy:.1f}%{replay_info}")
                ...
                for name, param in model.named_parameters():
                    if param.requires_grad:
                        if name not in continual_learner.fisher_dict:
                            continual_learner.fisher_dict[name] = torch.zeros_like(param)
        ...
        with torch.no_grad():
            for cluster_id in range(self.n_clusters):
                ...
                print(f"  Cluster {cluster_id}: {accuracy:>6.1f}% ({correct:>4}/{len(cluster_indices):>4})")
