...
def generate_synthetic_clusters(
    n_clusters: int, samples_per_cluster: int = 500, embedding_dim: int = 768
):
...
def train_with_dynamic_scaling(
    embeddings, labels, n_clusters, memory_budget_mb=1024, target_accuracy=0.80
):
    """
    ...
    buffer = DynamicHybridBuffer(initial_config=initial_config, auto_adapt=True)
    ...
    for cluster_id in range(n_clusters):
        ...
        for epoch in range(epochs_per_cluster):
            ...
            if cluster_id > 0:
                replay_X, replay_y = buffer.buffer.sample_replay_batch(
                    batch_size // 2, exclude_cluster=cluster_id
                )
        ...
        buffer.add_cluster(
            cluster_id, cluster_embeddings, cluster_labels, logits=cluster_logits
        )
...
def main():
    ...
    for scale_config in scales:
        ...
        embeddings, labels = generate_synthetic_clusters(
            n_clusters=n_clusters, samples_per_cluster=samples
        )
    ...
    print(f"\n{'─'*80}")
    print(
        f"{'Scale':<20} {'Clusters':<12} {'Accuracy':<12} {'Adaptations':<14} {'Memory':<12}"
    )
    print(f"{'─'*80}")
    ...
    for name, result in results.items():
        ...
        print(
            f"{name:<20} {n_clusters:<12} {accuracy:>6.1f}%     {adaptations:<14} {memory:>7.1f} MB"
        )
    ...
    print(
        f"   • Accuracy degradation: {baseline_acc:.1f}% → {medium_acc:.1f}% → {large_acc:.1f}%"
    )
    print(
        f"   • Degradation rate: {(baseline_acc - large_acc) / (50 - 5):.2f}pp per cluster"
    )
