...
def run_real_pipeline_simulation():
    ...
    for cluster_id in range(best_k):
        ...
        with torch.no_grad():
            ...
            true_labels = torch.full((len(cluster_embeddings),), cluster_id, dtype=torch.long)
    ...
    print(f"   Memory usage: {training_results['memory_mb']:.1f} MB / {pipeline.memory_budget_gb*1024:.0f} MB")
    print(f"   Usage: {100*training_results['memory_mb']/(pipeline.memory_budget_gb*1024):.1f}%")
    ...
    if training_results['adaptations'] > 0:
        print(f"   ✓ System adapted {training_results['adaptations']} time(s) during training.")
    else:
        print("   ℹ️ No adaptations needed for this scale.")
    
    if training_results['memory_mb'] / (pipeline.memory_budget_gb*1024) > 0.8:
        print("   ⚠️ High memory usage - consider increasing budget for larger datasets.")
    else:
