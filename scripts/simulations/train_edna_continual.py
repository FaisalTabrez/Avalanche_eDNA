...
class EDNAContinualTrainer:
    ...
    def initialize_model(self):
        ...
        try:
            ...
            self.continual_learner = ContinualLearner(
                strategy="combined", buffer_size=200, ewc_lambda=1000.0
            )
    ...
    def train_on_cluster(
        self, cluster_id: int, epochs: int = 10, batch_size: int = 16
    ) -> Dict:
        """
        ...
        for epoch in range(epochs):
            ...
            for i in range(0, cluster_size, batch_size):
                ...
                if cluster_id > 0 and self.continual_learner.strategy in [
                    "ewc",
                    "combined",
                ]:
                ...
                if self.continual_learner.strategy in ["replay", "combined"]:
                    ...
                    self.continual_learner.replay_buffer.add_samples(
                        batch_samples_str, batch_labels
                    )
        ...
        if self.continual_learner.strategy in ["ewc", "combined"]:
            ...
            try:
                self.continual_learner.compute_fisher_information(
                    self.model, fisher_loader, device=self.device
                )
            except Exception as e:
    ...
    def evaluate_on_all_clusters(self) -> Dict:
        ...
        with torch.no_grad():
            for cluster_id in range(self.n_clusters):
                ...
                print(
                    f"  Cluster {cluster_id}: Acc={accuracy:.2f}%, Loss={loss:.4f}, Size={total}"
                )
    ...
    def visualize_training(self):
        ...
        plt.tight_layout()
        plt.savefig(
            self.output_dir / "visualizations" / "cluster_performance.png", dpi=150
        )
        plt.close()
    ...
    def run_continual_training(self, epochs_per_cluster: int = 10):
        ...
        print(
            f"  • Replay buffer size: {len(self.continual_learner.replay_buffer.sequences)}"
        )
        ...
        for cluster_id in range(self.n_clusters):
            ...
            print(
                f"  Cluster {cluster_id}: {metrics['accuracy']:.1f}% "
