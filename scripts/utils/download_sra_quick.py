...
def download_sra(accession: str, output_base: str = "data/sra"):
    ...
    success, file_path = sra_ui.download_sra_dataset(
        accession, output_dir, progress_callback=lambda msg: print(f"  > {msg}")
    )
    ...
    if success and file_path:
        ...
        print("\n[TROUBLESHOOTING]")
        print(
            f"1. Verify accession exists: https://www.ncbi.nlm.nih.gov/sra/?term={accession}"
        )
        print(
            f"2. Check if it's a recent dataset (may not have FASTQ files on ENA yet)"
        )
