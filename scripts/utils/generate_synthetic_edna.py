...
def generate_dataset(
    organism_type: str, num_sequences: int, output_file: Path, start_id: int = 0
) -> int:
    """
    ...
    for i in range(num_sequences):
        ...
        # Generate sequence
        seq = generate_sequence(profile["gc_content"], length, profile["motifs"])
    ...
    print(
        f"    Length range: {min(len(s[1]) for s in sequences)}-{max(len(s[1]) for s in sequences)} bp"
    )
...
def main():
    ...
    for org_type in ORGANISM_PROFILES.keys():
        ...
        seq_id = generate_dataset(
            org_type, num_sequences=500, output_file=output_file, start_id=seq_id
        )
