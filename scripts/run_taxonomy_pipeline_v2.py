...
class TaxonomyClassificationPipeline:
    ...
    def __init__(
        ...
        for subdir in ['embeddings', 'clustering', 'taxonomy', 'models', 
                      'checkpoints', 'visualizations', 'reports']:
            (self.output_dir / subdir).mkdir(parents=True, exist_ok=True)
        
        # Load DNABERT-2
        logger.info(f"Loading DNABERT-2 from {dnabert_model_path}...")
        self.tokenizer = AutoTokenizer.from_pretrained(dnabert_model_path, trust_remote_code=True)
        self.dnabert_model = AutoModel.from_pretrained(dnabert_model_path, trust_remote_code=True)
        self.dnabert_model.to(device)
        self.dnabert_model.eval()
        logger.info(f"✓ DNABERT-2 loaded ({sum(p.numel() for p in self.dnabert_model.parameters()):,} parameters)")
        ...
        # Legacy continual learning (if dynamic scaling disabled)
            ...
            logger.info(f"✓ Legacy continual learning enabled (buffer={replay_buffer_size}, λ={ewc_lambda})")
    ...
    def load_sequences(self, fasta_file: str) -> List[Dict]:
        ...
        logger.info(f"  Length range: {min(s['length'] for s in sequences)}-{max(s['length'] for s in sequences)} bp")
    ...
    def generate_embeddings(self, batch_size: int = 32) -> np.ndarray:
        ...
        for i in range(0, total, batch_size):
            ...
            if (i + batch_size) % 320 == 0 or i + batch_size >= total:
                ...
                logger.info(f"  Progress: {progress:>5}/{total} ({100*progress/total:>5.1f}%) - {rate:.0f} seqs/sec")
        
        self.embeddings = np.vstack(embeddings)
        elapsed = time.time() - start_time
        
        logger.info(f"✓ Generated embeddings: {self.embeddings.shape}")
        logger.info(f"  Total time: {elapsed:.1f}s ({elapsed/total*1000:.1f}ms per sequence)")
    ...
    def cluster_sequences(self, n_clusters: int = 10, method: str = 'kmeans') -> np.ndarray:
        """
    ...
    def _train_with_dynamic_scaling(
        ...
        logger.info(f"Model: 768 -> {' -> '.join(map(str, config.hidden_dims))} -> {n_clusters}")
        ...
        for cluster_id in range(n_clusters):
            ...
            for epoch in range(epochs_per_cluster):
                ...
                for i in range(0, len(cluster_indices), config.batch_size):
                    ...
                    if cluster_id > 0:
                        ...
                        replay_X, replay_y = self.dynamic_buffer.buffer.sample_replay_batch(
                            replay_size,
                ...
                if (epoch + 1) % 2 == 0:
                    logger.info(f"  Epoch {epoch+1}/{epochs_per_cluster}: Loss={avg_loss:.4f}, Acc={accuracy:.1f}%")
            ...
            if new_config.n_clusters != prev_config.n_clusters:
                ...
                logger.info(f"   Clusters: {prev_config.n_clusters} → {new_config.n_clusters}")
                
                # Recreate model if architecture changed
                if new_config.hidden_dims != prev_config.hidden_dims:
                    logger.info(f"   Architecture: {prev_config.hidden_dims} → {new_config.hidden_dims}")
        ...
        logger.info(f"Adaptation history: {len(self.dynamic_buffer.adaptation_history)} events")
        logger.info(f"")
        ...
        logger.info(f"  Memory: {memory_mb:.1f} MB / {self.memory_budget_gb*1024:.0f} MB")
    ...
    def _train_with_legacy_continual_learning(
        ...
        logger.info(f"Strategy: {'Active' if use_active_replay else 'Passive'} Replay + EWC")
        ...
        for cluster_id in range(n_clusters):
            ...
            for epoch in range(epochs_per_cluster):
                ...
                for i in range(0, len(cluster_indices), batch_size):
                    ...
                    if use_active_replay and cluster_id > 0:
                        if len(self.continual_learner.replay_buffer.sequences) > 0:
                            ...
                            replay_samples = self.continual_learner.replay_buffer.sample(replay_size)
                            ...
                            if replay_samples and replay_samples[0]:
                                ...
                                replay_y = torch.LongTensor(replay_samples[1]).to(self.device)
                    ...
                    if cluster_id > 0:
                        ewc_loss = self.continual_learner.compute_ewc_loss(self.classifier_model)
                        loss = loss + ewc_loss
                    ...
                    batch_seqs_str = [str(batch_X[j].cpu().numpy().tolist()) for j in range(len(batch_X))]
                    batch_labels = batch_y.cpu().tolist()
                    self.continual_learner.replay_buffer.add_samples(batch_seqs_str, batch_labels)
                ...
                if (epoch + 1) % 2 == 0:
                    replay_info = f", Replay: {replay_batches_used}" if use_active_replay and cluster_id > 0 else ""
                    logger.info(f"  Epoch {epoch+1}/{epochs_per_cluster}: Loss={avg_loss:.4f}, Acc={accuracy:.1f}%{replay_info}")
        ...
        with torch.no_grad():
            for cluster_id in range(n_clusters):
                cluster_indices = np.where(self.cluster_labels == cluster_id)[0]
                X_test = torch.FloatTensor(self.embeddings[cluster_indices]).to(self.device)
                y_test = torch.full((len(cluster_indices),), cluster_id, dtype=torch.long).to(self.device)
                ...
                logger.info(f"  Cluster {cluster_id}: {accuracy:>6.1f}% ({correct:>4}/{len(cluster_indices):>4})")
    ...
    def assign_taxonomy(
        ...
        for i, seq_data in enumerate(self.sequences):
            ...
            if self.classifier_model is not None:
                self.classifier_model.eval()
                with torch.no_grad():
                    embedding = torch.FloatTensor(self.embeddings[i:i+1]).to(self.device)
    ...
    def generate_visualizations(self):
        ...
        plt.tight_layout()
        plt.savefig(self.output_dir / 'visualizations' / 'cluster_analysis.png', dpi=150)
        plt.close()
    ...
    def run_complete_pipeline(
        ...
        if train_classifier and 'overall_accuracy' in training_results:
            logger.info(f"Classifier accuracy: {training_results['overall_accuracy']:.1f}%")
...
def main():
    ...
    parser.add_argument(
        'input_fasta',
        type=str,
        help='Input FASTA file with eDNA sequences'
    )
    parser.add_argument(
        '--output-dir',
        type=str,
