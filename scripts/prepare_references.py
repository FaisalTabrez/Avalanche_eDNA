...
def iter_fasta_with_names(
    fasta: Optional[Path], taxonomy_df: Optional[pd.DataFrame], source: str
        ...
        for rec in SeqIO.parse(handle, "fasta"):
            ...
            if taxonomy_df is not None:
                ...
                if "sequence_id" in taxonomy_df.columns:
                    ...
                    if not row.empty:
                        ...
                        for col in [
                            ...
                            if col in taxonomy_df.columns and pd.notna(
                                row.iloc[0].get(col)
                            ):
                ...
                if name is None and "name" in taxonomy_df.columns:
                    ...
                    cand = taxonomy_df[
                        taxonomy_df["name"].astype(str).str.contains(rec.id, na=False)
                    ]
                    if not cand.empty:
...
def make_lineage_labels(
    resolver: TaxdumpResolver, name_to_taxid: Dict[str, int], name: Optional[str]
) -> Tuple[Optional[int], Dict[str, Optional[str]]]:
    taxid: Optional[int] = None
    if name:
        taxid = name_to_taxid.get(name.lower())
    lineage = (
        resolver.lineage_by_taxid(taxid)
        if taxid is not None
        else resolver.lineage_by_name(name if isinstance(name, str) else None)
    )
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Prepare unified marine eukaryote references (PR2 + SILVA)"
    )
    ...
    if not resolver.available():
        raise RuntimeError(
            f"Taxdump not available at {taxdump_dir} (names.dmp and nodes.dmp are required)"
        )
    name2taxid = load_name_to_taxid(taxdump_dir)

    pr2_tax = (
        read_taxonomy_table(Path(args.pr2_taxonomy)) if args.pr2_taxonomy else None
    )
    silva_tax = (
        read_taxonomy_table(Path(args.silva_taxonomy)) if args.silva_taxonomy else None
    )

    recs: List[Tuple[str, str, Optional[str]]] = []
    recs += iter_fasta_with_names(
        Path(args.pr2_fasta) if args.pr2_fasta else None, pr2_tax, source="pr2"
    )
    recs += iter_fasta_with_names(
        Path(args.silva_fasta) if args.silva_fasta else None, silva_tax, source="silva"
    )

    if not recs:
        raise RuntimeError("No reference sequences found. Provide at least one FASTA.")

    recs = filter_and_dedupe(
        recs, max_n_fraction=float(args.max_n_fraction), min_length=int(args.min_length)
    )
    ...
    taxid_map_out = (
        Path("reference") / "mappings" / f"combined_{args.marker}_taxid_map.txt"
    )
    ...
    df["name"] = df.apply(
        lambda x: x["name"] if pd.notna(x["name"]) and x["name"] else x.get("species"),
        axis=1,
    )
