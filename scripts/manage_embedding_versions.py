...
def register_version(
    version: str, model: str, description: str = "", config: Optional[Dict] = None
):
...
def main():
    parser = argparse.ArgumentParser(description="Manage embedding model versions")
    subparsers = parser.add_subparsers(dest="command", help="Command to execute")

    # Register command
    register_parser = subparsers.add_parser("register", help="Register a new version")
    register_parser.add_argument(
        "--version", required=True, help="Version name (e.g., v1.0)"
    )
    register_parser.add_argument("--model", required=True, help="Model name")
    register_parser.add_argument(
        "--description", default="", help="Version description"
    )
